...
public class ProductStoreWorker {
    ...
    public static GenericValue getProductStorePaymentSetting(Delegator delegator, String productStoreId, String paymentMethodTypeId, String paymentServiceTypeEnumId, boolean anyServiceType) {
        ...
        //System.out.println("###############        storePayment         "+storePayment);
        if (anyServiceType) {
    ...
    public static List<GenericValue> getProductStoreShipmentMethods(Delegator delegator, String productStoreId,
        ...
        try {
        	Debug.log("\n\n <ProductStoreShipmentMeth storeFields == "+storeFields+"\n\n");
    ...
    public static List<GenericValue> getAvailableStoreShippingMethods(Delegator delegator, String productStoreId, GenericValue shippingAddress, List<BigDecimal> itemSizes, Map<String, BigDecimal> featureIdMap, BigDecimal weight, BigDecimal orderTotal) {
        if (featureIdMap == null) {
            featureIdMap = new HashMap(); //FastMap.newInstance();
        }
    ...
    public static List<GenericValue> getSurveys(Delegator delegator, String productStoreId, String groupName, String productId, String surveyApplTypeId, String parentProductId) {
        List<GenericValue> surveys =new ArrayList<GenericValue>(); //FastList.newInstance();
    ...
    public static boolean isStoreInventoryAvailable(ServletRequest request, String productId) {
    	Delegator delegator = (Delegator)request.getAttribute("delegator");
    	GenericValue product = null;
		try {
		...
		if(UtilValidate.isEmpty(product)) return false;
        return isStoreInventoryAvailable(request, product, java.math.BigDecimal.ONE);
    }
    ...
    protected static Map<String, String> defaultProductStoreEmailScreenLocation = new HashMap<String, String>(); //FastMap.newInstance();
    ...
    public static String isInventoryAvailableForProduct(HttpServletRequest request,HttpServletResponse response){
    	String productId = request.getParameter("productId");
    	
    	boolean flag = false;
    	try {
    		flag = isStoreInventoryAvailable(request, 
					((GenericDelegator) request.getAttribute("delegator")).findOne("Product", UtilMisc.toMap("productId",productId), false), BigDecimal.ONE);
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
    	...
    	return "success";
    	//isStoreInventoryAvailable(request, product, 1.0?double)
    }
}
