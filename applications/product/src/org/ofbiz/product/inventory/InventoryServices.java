...
public class InventoryServices {
    ...
    public static Map<String, Object> checkInventoryAvailability(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        */
        ...
        Map<String, Map<String, Timestamp>> ordersToUpdate = new HashMap<String, Map<String, Timestamp>>(); //FastMap.newInstance();
        Map<String, Map<String, Timestamp>> ordersToCancel =  new HashMap<String, Map<String, Timestamp>>();//FastMap.newInstance();
        ...
        for (GenericValue inventoryItem: inventoryItems) {
            ...
            try {
                List<EntityExpr> exprs = new ArrayList<EntityExpr>();//FastList.newInstance();
            ...
            for (GenericValue reservation: reservations) {
                ...
                if (nextShipDate == null || nextShipDate.after(actualPromiseDate)) {
                    if (nextShipDate == null && modifiedPromisedDate.after(now)) {
                        ...
                        if (notifyItems == null) {
                            notifyItems = new HashMap<String, Timestamp>(); //FastMap.newInstance();
                        }
                        ...
                        if (needToCancel) {
                            ...
                            if (cancelItems == null) {
                                cancelItems = new HashMap<String, Timestamp>(); //FastMap.newInstance();
                            }
        ...
        List<String> ordersToNotify =new ArrayList<String>(); //FastList.newInstance();
        for (Map.Entry<String, Map<String, Timestamp>> entry: ordersToUpdate.entrySet()) {
            ...
            for (GenericValue orderItemShipGroup: orderItemShipGroups) {
                List<GenericValue> orderItems = new ArrayList<GenericValue>();//FastList.newInstance();
                ...
                if (cancelItems == null) {
                    cancelItems = new HashMap(); //FastMap.newInstance();
                }
    ...
    public static Map<String, Object> getProductInventorySummaryForItems(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        Map<String, BigDecimal> atpMap = new HashMap<String, BigDecimal>(); //FastMap.newInstance();
        Map<String, BigDecimal> qohMap = new HashMap<String, BigDecimal>(); //FastMap.newInstance();
        Map<String, BigDecimal> mktgPkgAtpMap = new HashMap<String, BigDecimal>(); //FastMap.newInstance();
        Map<String, BigDecimal> mktgPkgQohMap = new HashMap<String, BigDecimal>(); //FastMap.newInstance();
        Map<String, Object> results = ServiceUtil.returnSuccess();
    ...
    public static Map<String, Object> getProductInventoryAndFacilitySummary(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        Map<String, Object> resultOutput = new HashMap<String, Object>(); //FastMap.newInstance();
     ...
     * @Author  Ajaya Dash
     */
 public static  Map<String, Object> autoInventoryTransferFromOneProductToAnother(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        final String facilityId = UtilProperties.getPropertyValue("general.properties", "posstore.usefaclityId");
        ...
        */
        ...
        List<EntityCondition> conditionList =new ArrayList<EntityCondition>();// FastList.newInstance();
        
		conditionList.add(EntityCondition.makeCondition("productCategoryId", EntityOperator.EQUALS, "FF"));
		conditionList.add(EntityCondition.makeCondition("thruDate", EntityOperator.EQUALS, null));
		 ...
		 List<GenericValue> productCategoryMemberList = null;
	        try 
	        {
	        	productCategoryMemberList = delegator.findList("ProductCategoryMember", EntityCondition.makeCondition(conditionList, EntityOperator.AND), null, null, null, false);
			} catch (GenericEntityException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
       ...
       // for(GenericValue facility : facilityList){
        //	String facilityId = (String) facility.get("facilityId");
        	...
        	for(GenericValue productCategoryMember : productCategoryMemberList){
        		String productId = (String) productCategoryMember.get("productId");
...
/*        		GenericValue product = null;
        		...
        		if(product.get("transferInventoryToProductId") == null)
        			continue;
*/        		Map invetorySeq = null;
    			
    			try {
					invetorySeq = findTranserInventorySeq(productId,delegator);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
    			
    			int size = invetorySeq.size();
    			
    			if(size < 2)return results;
    			
    			productId = (String)invetorySeq.get(size+"");
        		
    			try {
					results = damageInventory(facilityId,productId,userLogin,dispatcher);
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
    		 	String productIdFrom = null;
    		 	String productIdTo = null;
    			for(int i=size-1;i>0;i--)
    			{
    				productIdFrom =   (String)invetorySeq.get(i+"");
    				productIdTo =   (String)invetorySeq.get(i+1+"");
    				
    				try {
    					if(i == 1)
    					{
    						transferFirst(facilityId,productIdFrom,productIdTo,daysLater,userLogin,delegator,dispatcher);
    					}else
    					{
    						transfer(facilityId,productIdFrom,productIdTo,daysLater,userLogin,delegator,dispatcher);
    					}
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
	 ...
	 public static Map<String,String> findTranserInventorySeq(String productId,GenericDelegator delegator) throws Exception
		{
		 	Map<String,String> inventoryTransferSeq = new HashMap<String, String>();
			GenericValue product = null;
	 		try {
	 		...
	 		if(product != null)
	 		{
	 			inventoryTransferSeq.put(seq+"", productId);
	 			seq += 1;
	 		}
	 			for(;;)
	 			{
	 				...
	 				String transferInventoryTo = (String) product.get("transferInventoryToProductId");
	 				
		 			if(transferInventoryTo == null)break;
		 			
		 			inventoryTransferSeq.put(seq+"", transferInventoryTo);
		 			seq += 1;
		 			try {
		 				product =  ProductWorker.findProduct(delegator,transferInventoryTo);
					} catch (GenericEntityException e) {
						// TODO Auto-generated catch block
						seq -= 1;
						e.printStackTrace();
	 			...
	 			return inventoryTransferSeq;
		}
	 
        public static Map<String, Object> performFind(String facilityId,String productId,String dateTimeValue,String dateTimeOp,String isTransforableToOld ,LocalDispatcher dispatcher) throws Exception
     	{
        	...
        	param.put("datetimeReceived_fld0_value", dateTimeValue);
        	param.put("datetimeReceived_fld0_op", dateTimeOp);
        	
        	//param.put("datetimeReceived_fld1_value", "");
        	//param.put("datetimeReceived_fld0_op", "lessThan");
        	
        	param.put("internalName", "");
        	param.put("inventoryItemId", "");
        	param.put("statusId", "");
        	param.put("serialNumber", "");
        	param.put("softIdentifier", "");
        	param.put("partyId", "");
        	param.put("lotId", "");
        	param.put("isTransforableToOld", isTransforableToOld);
        	...
        	performFind.put("entityName", "InventoryItemAndLocation");
        	performFind.put("orderBy", "statusId|quantityOnHandTotal|serialNumber");
        	performFind.put("entityName", "InventoryItemAndLocation");
       ...
       /* 
        public static Map<String, Object> adjustATPAndReceiveInventory(DispatchContext dctx, Map<String, ? extends Object> context) {
            GenericDelegator delegator = (GenericDelegator) dctx.getDelegator();
            LocalDispatcher dispatcher = dctx.getDispatcher();
            GenericValue userLogin = (GenericValue) context.get("userLogin");
            String productId = (String) context.get("productId");
            ...
            String isVirtualStock = (String) context.get("isVirtualStock");
            if(UtilValidate.isEmpty(isVirtualStock) || !"Y".equalsIgnoreCase(isVirtualStock))
            	isVirtualStock = "N";
            
            boolean errorFlag = adjustATPAndReceiveInventory(delegator, dispatcher, productId, userLogin, facilityId, quantity, unitCost, isVirtualStock);

            if(errorFlag)
    			return ServiceUtil.returnError("failed To Receive Inventory");

    		return ServiceUtil.returnSuccess();
        }
        */
        /*public static boolean adjustATPAndReceiveInventory(Delegator delegator, LocalDispatcher dispatcher, String productId, 
        		GenericValue userLogin, String facilityId, BigDecimal quantity, BigDecimal unitCost, String isVirtualStock){
        	...
        	boolean errorFlag = adjustATP(delegator, dispatcher, productId, userLogin);
        	if(!errorFlag)
        	{
	        	String message = receiveInventory(dispatcher, delegator,productId, facilityId, userLogin, quantity, unitCost, isVirtualStock);
        ...
        public static Map<String, Object> adjustATP(DispatchContext dctx, Map<String, ? extends Object> context) {
            GenericDelegator delegator = (GenericDelegator) dctx.getDelegator();
            LocalDispatcher dispatcher = dctx.getDispatcher();
            GenericValue userLogin = (GenericValue) context.get("userLogin");
            String productId = (String) context.get("productId");
    		...
    		if(errorFlag)
    			return ServiceUtil.returnError("failed To Adjust ATP");

    		BigDecimal QOH = ProductWorker.totalAvailableQOH(delegator, dispatcher, facilityId, productId, null);
    		if(UtilValidate.isEmpty(QOH)) QOH = BigDecimal.ZERO;
    		Map<String, Object> result = UtilMisc.toMap("QOH",(Object)QOH);
    		return result;
        }
        
        public static boolean adjustATP(Delegator delegator, LocalDispatcher dispatcher, String productId, GenericValue userLogin){
        	
        	List condition = UtilMisc.toList(EntityCondition.makeCondition("productId",EntityOperator.EQUALS,productId),
        									 EntityCondition.makeCondition("availableToPromiseTotal",EntityOperator.GREATER_THAN,BigDecimal.ZERO));
        	
        	List<GenericValue> inventoryItems = null;
        	try {
				inventoryItems = delegator.findList("InventoryItem",
														EntityCondition.makeCondition(condition,EntityOperator.AND),null,UtilMisc.toList("datetimeReceived DESC"),null,true);
			} catch (GenericEntityException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			...
			for(GenericValue inventoryItem : inventoryItems)
			{
				//atp = ProductWorker.totalAvailableATP(delegator, dispatcher, facilityId, productId);
				String inventoryItemId = inventoryItem.getString("inventoryItemId");
				
				BigDecimal atp = ProductWorker.totalAvailableATP(delegator, dispatcher, inventoryItem.getString("facilityId"), productId, inventoryItemId);
				String message = adjustInventoryATP(userLogin, inventoryItemId, atp, dispatcher, productId);
				if(!"success".equals(message))
				{
					errorFlag = true;
					break;
        ...
        public static String adjustInventoryATP(GenericValue userLogin,String inventoryItemId,BigDecimal availableToPromiseTotal, 
        																								LocalDispatcher dispatcher, String productId)
	 	{
	 		Map<String, Object> adjust =  ServiceUtil.returnSuccess();
	 		adjust.remove("responseMessage");
	
	 		adjust.put("userLogin", userLogin);
	 		adjust.put("inventoryItemId", inventoryItemId);
	 		adjust.put("varianceReasonId", "VAR_INV_ADJUST");
	 		adjust.put("availableToPromiseVar", availableToPromiseTotal.multiply(new java.math.BigDecimal(-1)));
			adjust.put("comments", "Old ATP adjusting For product "+productId);
			try {
				adjust = dispatcher.runSync("createPhysicalInventoryAndVariance", adjust);
			} catch (GenericServiceException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			if (ServiceUtil.isError(adjust) || ServiceUtil.isFailure(adjust)) {
            	return ServiceUtil.getErrorMessage(adjust);
            }
            return "success";
	 	}
        
       /* public static String receiveInventory(LocalDispatcher dispatcher, Delegator delegator,String productId, String facilityId, 
        												GenericValue userLogin, BigDecimal quantity, BigDecimal unitCost, String isVirtualStock){
            // assign item to package
        	
        	if(UtilValidate.isEmpty(facilityId)) facilityId = "WebStoreWarehouse";
        	
        	String comments = "Receive QOH From Excell Uploading for productId "+productId+" for facility "+facilityId;
        	
        	Map<String, Object> inventoryMap = FastMap.newInstance();
        	
        	if("Y".equalsIgnoreCase(isVirtualStock))
        	{
        		inventoryMap.put("virtualAccepted", quantity);
        		quantity = BigDecimal.ZERO;
        	}
        	inventoryMap.put("quantityAccepted", quantity);
        	inventoryMap.put("quantityRejected", BigDecimal.ZERO);
        	inventoryMap.put("inventoryItemTypeId", "NON_SERIAL_INV_ITEM");
        	inventoryMap.put("unitCost", unitCost);
        	inventoryMap.put("productId", productId);
        	inventoryMap.put("facilityId", facilityId);
        	inventoryMap.put("comments", comments);
			...
			try {
				packageResp = dispatcher.runSync("receiveInventoryProductFromPacking", inventoryMap);
			} catch (GenericServiceException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			
            if (ServiceUtil.isError(packageResp) || ServiceUtil.isFailure(packageResp)) {
            	return ServiceUtil.getErrorMessage(packageResp);
            }
            return "succes";
        }*/
        
	 	public static Map<String, Object> transferInventory(GenericValue userLogin,String inventoryItemId,BigDecimal availableToPromiseTotal,LocalDispatcher dispatcher,GenericValue productFrom,GenericValue productto) throws Exception
	 	{
	 		Map<String, Object> transfer =  ServiceUtil.returnSuccess();
			transfer.remove("responseMessage");
	
			transfer.put("userLogin", userLogin);
			transfer.put("inventoryItemId", inventoryItemId);
			transfer.put("varianceReasonId", "VAR_TRANSFER");
			transfer.put("availableToPromiseVar", availableToPromiseTotal.multiply(new java.math.BigDecimal(-1)));
			transfer.put("quantityOnHandVar", availableToPromiseTotal.multiply(new java.math.BigDecimal(-1)));
			transfer.put("comments", "Transfered from "+productFrom.get("internalName")+"["+productFrom.get("productId")+"] to "+productto.get("internalName")+"["+productto.get("productId")+"]");
			
			
			
			transfer = dispatcher.runSync("createPhysicalInventoryAndVariance", transfer);
			
			return transfer;
	 	}
	 	public static Map<String, Object> receiveInventory(GenericValue userLogin,String inventoryItemId,BigDecimal availableToPromiseTotal,LocalDispatcher dispatcher,GenericValue productFrom,GenericValue productto) throws Exception
	 	{
	 		
	 		Map<String, Object> receive =  ServiceUtil.returnSuccess();
	 		receive.remove("responseMessage");
	
	 		receive.put("userLogin", userLogin);
	 		receive.put("inventoryItemId", inventoryItemId);
	 		receive.put("varianceReasonId", "VAR_RECEIVE");
	 		receive.put("availableToPromiseVar", availableToPromiseTotal/*.multiply(new java.math.BigDecimal(-1))*/);
	 		receive.put("quantityOnHandVar", availableToPromiseTotal/*.multiply(new java.math.BigDecimal(-1))*/);
	 		receive.put("comments", "Received From "+productFrom.get("internalName")+"["+productFrom.get("productId")+"] to "+productto.get("internalName")+"["+productto.get("productId")+"]");
			
	 		receive = dispatcher.runSync("createPhysicalInventoryAndVariance", receive);
	 		
	 		return receive;
	 	}
	 	public static Map<String, Object> damageInventory(String facilityId, String productId,GenericValue userLogin,LocalDispatcher dispatcher) throws Exception
	 	{
	 		Map<String, Object> results =  ServiceUtil.returnSuccess();
	 		try {
				results = performFind(facilityId,productId, "","equals","Y",dispatcher);
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
	 		org.ofbiz.entity.util.EntityListIterator listIt = (org.ofbiz.entity.util.EntityListIterator)results.get("listIt");
	 		String inventoryItemId = null;
	 		java.math.BigDecimal availableToPromiseTotal = null;
	 		for(;;)
	 		{
	 			GenericValue inventory = listIt.next();
	 			if(inventory == null) break;
    			inventoryItemId = (String)inventory.get("inventoryItemId");
    			availableToPromiseTotal = (java.math.BigDecimal)inventory.get("availableToPromiseTotal");
	 		}
	 		Map<String, Object> receive =  ServiceUtil.returnSuccess();
	 		receive.put("responseMessage", "inventoryItemId or availableToPromiseTotal is null");
	 		
	 		if(inventoryItemId == null && availableToPromiseTotal ==null)return receive;
	 		
	 		receive.remove("responseMessage");
	
	 		receive.put("userLogin", userLogin);
	 		receive.put("inventoryItemId", inventoryItemId);
	 		receive.put("varianceReasonId", "VAR_DAMAGED");
	 		receive.put("availableToPromiseVar", availableToPromiseTotal.multiply(new java.math.BigDecimal(-1)));
	 		receive.put("quantityOnHandVar", availableToPromiseTotal.multiply(new java.math.BigDecimal(-1)));
	 		receive.put("comments", "Damaged");
	 	...
	 	public static Map<String, Object> transferFirst(String facilityId,String productIdFrom,String productIdTo,int daysLater,GenericValue userLogin,GenericDelegator delegator,LocalDispatcher dispatcher) throws Exception
	 	{
	 		
	 		Map<String, Object> results =  ServiceUtil.returnSuccess();
	 		
	 		Map<String, Object> from =  ServiceUtil.returnSuccess();
	 		Map<String, Object> to =  ServiceUtil.returnSuccess();
	 		
	 		GenericValue fromProduct = null;
	 		GenericValue toProduct = null;
 			
	 		fromProduct =  ProductWorker.findProduct(delegator,productIdFrom);
 			toProduct =  ProductWorker.findProduct(delegator,productIdTo);
			
 			//from = performFind(facilityId,productIdFrom, UtilDateTime.getDayStart(UtilDateTime.nowTimestamp(), daysLater)+"","sameDay",null,dispatcher);
 			from = performFind(facilityId,productIdFrom, "","equals","Y",dispatcher);
			to = performFind(facilityId,productIdTo, "","equals","Y",dispatcher);
			
        	
    		org.ofbiz.entity.util.EntityListIterator fromListIt = (org.ofbiz.entity.util.EntityListIterator)from.get("listIt");
    		org.ofbiz.entity.util.EntityListIterator toListIt = (org.ofbiz.entity.util.EntityListIterator)to.get("listIt");

    		String fromInventoryItemId = null;
    		String toInventoryItemId = null;
    		
    		java.math.BigDecimal fromAvailableToPromiseTotal = BigDecimal.ZERO;
    		java.math.BigDecimal toAvailableToPromiseTotal = null;
    		
        	if(fromListIt != null)
        	{
        		for(;;)
    	 		{
	 			GenericValue inventory = fromListIt.next();
	 			if(inventory == null) break;
        		if(inventory != null)
        		{
    			fromInventoryItemId = (String)inventory.get("inventoryItemId");
    			fromAvailableToPromiseTotal = fromAvailableToPromiseTotal.add((java.math.BigDecimal)inventory.get("availableToPromiseTotal"));
        		}
    			results = transferInventory(userLogin,fromInventoryItemId,(java.math.BigDecimal)inventory.get("availableToPromiseTotal"),dispatcher,fromProduct,toProduct);
        		}
        	}
        	if(toListIt != null)
        	{
        		GenericValue inventory = toListIt.next();
        		if(inventory != null)
        		{
    			toInventoryItemId = (String) inventory.get("inventoryItemId");
        		}
        	}
			results = receiveInventory(userLogin,toInventoryItemId,fromAvailableToPromiseTotal,dispatcher,fromProduct,toProduct);
			
			
			if(fromListIt != null)fromListIt.close();
			if(toListIt != null)toListIt.close();

			return results;
	 	}

	 	public static Map<String, Object> transfer(String facilityId,String productIdFrom,String productIdTo,int daysLater,GenericValue userLogin,GenericDelegator delegator,LocalDispatcher dispatcher) throws Exception
	 	{
	 		Map<String, Object> results =  ServiceUtil.returnSuccess();
    		...
    		java.math.BigDecimal fromAvailableToPromiseTotal = null;
    		java.math.BigDecimal toAvailableToPromiseTotal = null;
        	...
        	if(fromListIt != null)
        	{
        		...
        		{
    			fromInventoryItemId = (String)inventory.get("inventoryItemId");//
    			fromAvailableToPromiseTotal = (java.math.BigDecimal)inventory.get("availableToPromiseTotal");
        		}
        	}
        	if(toListIt != null)
        	{
        		...
        		{
    			toInventoryItemId = (String)inventory.get("inventoryItemId");
    			toAvailableToPromiseTotal = fromAvailableToPromiseTotal;//(java.math.BigDecimal)inventory.get("availableToPromiseTotal");
        		}
        	}
			results = transferInventory(userLogin,fromInventoryItemId,fromAvailableToPromiseTotal,dispatcher,fromProduct,toProduct);
			results = receiveInventory(userLogin,toInventoryItemId,toAvailableToPromiseTotal,dispatcher,fromProduct,toProduct);
	 	...
	 	public static Map<String, Object> updateInventoryInProduct(DispatchContext dctx, Map<String, ? extends Object> context) {
	 		try {
	 		Delegator delegator = dctx.getDelegator();
	        LocalDispatcher dispatcher = dctx.getDispatcher();
	 		...
	 		GenericValue inventoryItem = null;
	 			...
	 			inventoryItem = delegator.findOne("InventoryItem", true, UtilMisc.toMap("inventoryItemId",inventoryItemId));
			...
			updateInventoryInProduct(delegator, dispatcher, productId, facilityId);
	 		} catch (GenericEntityException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	 	...
	 	public static void updateInventoryInProduct(Delegator delegator, LocalDispatcher dispatcher, String productId, String facilityId) {
	 		try {
	 			TransactionUtil.begin();
			} catch (GenericEntityException e) {
				// TODO Auto-generated catch block
			...
			product = ProductWorker.getParentProduct(productId, delegator);
																							...
																							"Y".equalsIgnoreCase(product.getString("isVirtual")))
			{
				...
				List<GenericValue> virtualProductAssocs = null;
				...
				cond.add(EntityCondition.makeCondition("productAssocTypeId", "PRODUCT_VARIANT"));
				...
				try {
					virtualProductAssocs = delegator.findList("ProductAssoc", EntityCondition.makeCondition(cond,EntityOperator.AND), 
																UtilMisc.toSet("productIdTo"), UtilMisc.toList("sequenceNum"), null, false);
				...
				{
					productId = virtualProductAssoc.getString("productIdTo");
					if(UtilValidate.isNotEmpty(productId))
					{
						atp = ProductWorker.totalAvailableATP(delegator, dispatcher, facilityId, productId, null);
						if(atp.doubleValue() >=1 )
							break;
					}
				}
				
				try {
					product = delegator.findOne("Product", false, UtilMisc.toMap("productId", product.getString("productId")));
					if(UtilValidate.isNotEmpty(product))
					{
			 			product.put("inventoryAtp", atp);
			 			product.store();
