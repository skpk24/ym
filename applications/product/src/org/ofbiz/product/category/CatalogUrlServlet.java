...
public class CatalogUrlServlet extends HttpServlet {
    ...
    public static String makeCatalogUrl(HttpServletRequest request, String productId, String currentCategoryId, String previousCategoryId,String previousPreCategoryId) {
        StringBuilder urlBuilder = new StringBuilder();
        urlBuilder.append(request.getSession().getServletContext().getContextPath());
        if (urlBuilder.charAt(urlBuilder.length() - 1) != '/') {
            urlBuilder.append("/");
        ...
        if (UtilValidate.isNotEmpty(currentCategoryId)) {
            List<String> trail = CategoryWorker.getTrail(request);
            trail = CategoryWorker.adjustTrail(trail, currentCategoryId, previousCategoryId);
            if(UtilValidate.isNotEmpty(previousPreCategoryId))
            	urlBuilder.append("/"+previousPreCategoryId);
            for (String trailCategoryId: trail) {
                if ("TOP".equals(trailCategoryId)) continue;
                urlBuilder.append("/");
                urlBuilder.append(trailCategoryId);
    ...
    public static String makeCatalogUrl(HttpServletRequest request, String productId, String currentCategoryId, String previousCategoryId,String previousPreCategoryId,String previousPrePreCategoryId) {
        StringBuilder urlBuilder = new StringBuilder();
        urlBuilder.append(request.getSession().getServletContext().getContextPath());
        if (urlBuilder.charAt(urlBuilder.length() - 1) != '/') {
            urlBuilder.append("/");
        ...
        if (UtilValidate.isNotEmpty(currentCategoryId)) {
            ...
            if(UtilValidate.isNotEmpty(previousPrePreCategoryId))
            	urlBuilder.append("/"+previousPrePreCategoryId);
            if(UtilValidate.isNotEmpty(previousPreCategoryId))
            	urlBuilder.append("/"+previousPreCategoryId);
            for (String trailCategoryId: trail) {
                if ("TOP".equals(trailCategoryId)) continue;
