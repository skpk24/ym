...
public class CatalogWorker {
    ...
    public static List<String> getCatalogIdsAvailable(List<GenericValue> partyCatalogs, List<GenericValue> storeCatalogs) {
        ...
        */
        ...
        List<GenericValue> allCatalogLinks = new ArrayList<GenericValue>();
    ...
    public static Collection<String> getCatalogQuickaddCategories(ServletRequest request, String prodCatalogId) {
      ...
      //  Collection<String> categoryIds = FastList.newInstance();
        Collection<String> categoryIds = new ArrayList<String>();
    ...
    public static Map<String,List<GenericValue>>  getBannerCategory(ServletRequest request) {
//        if (prodCatalogId == null || prodCatalogId.length() <= 0) return null;
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        ...
        List<GenericValue> productCatrgoryValues = new ArrayList<GenericValue>();
        try {
			 productCatrgoryValues  = delegator.findList("BannerManagement", EntityCondition.makeCondition("position",EntityOperator.EQUALS,"CATEGORY"), null, UtilMisc.toList("sequenceNum"), findOpts,true);
		} catch (GenericEntityException e) {
 			e.printStackTrace();
		}
        productCatrgoryValues = EntityUtil.filterByDate(productCatrgoryValues);
        Map<String,List<GenericValue>> productCatrgoryMapValues = new HashMap<String,List<GenericValue>>();
        for(GenericValue gv : productCatrgoryValues){
        	String groupName = gv.getString("groupName");
        	if(productCatrgoryMapValues.containsKey(groupName)){
        		List<GenericValue> l = (List) productCatrgoryMapValues.get(groupName);
        		l.add(gv);
        		productCatrgoryMapValues.put(groupName, l);
        	}else{
       ...
       return  productCatrgoryMapValues;
    }
}
