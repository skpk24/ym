...
public class ProductWorker {
    ...
    public static List<String> getProductIDs(Delegator delegator, String  categoryId) throws GenericEntityException {
        //GenericValue aggregatedProduct = delegator.findByPrimaryKey("ProductCategoryMember", UtilMisc.toMap("productId", categoryId));
		...
		cond.add(EntityCondition.makeCondition("productCategoryId",EntityOperator.EQUALS,categoryId));

        pp = delegator.findList("ProductAndCategoryMember",EntityCondition.makeCondition(cond,EntityOperator.AND), null, null, null, false);
        
        if(UtilValidate.isNotEmpty(pp))
        for(GenericValue productfea : pp)
	   	{
	   		 String productId= (String) productfea.get("productId");
    ...
    public static List<GenericValue> getProductFeaturesByApplTypeId(GenericValue product, String productFeatureApplTypeId) {
        ...
        try {
            ...
            Debug.logError(e, module);
            features =new ArrayList<GenericValue>();// FastList.newInstance();
        }
        return features;
    }
    ...
    public static List<List<Map<String,String>>> getSelectableProductFeaturesByTypesAndSeq(GenericValue product) {
        ...
        List <List<Map<String,String>>> featureTypeFeatures = new ArrayList<List<Map<String,String>>>();//FastList.newInstance();
        try {
            Delegator delegator = product.getDelegator();
            ...
            String oldType = null;
            List<Map<String,String>> featureList = new ArrayList<Map<String,String>>();//FastList.newInstance();
            for (GenericValue productFeatureAppl: featuresSorted) {
                if (oldType == null || !oldType.equals(productFeatureAppl.getString("productFeatureTypeId"))) {
                    ...
                    if (oldType != null) {
                        ...
                        featureList =new ArrayList<Map<String,String>>(); //FastList.newInstance();
                    }
    ...
    public static List<GenericValue> getVariantSelectionFeatures(GenericValue variantProduct) {
        ...
        List<GenericValue> result =new ArrayList<GenericValue>();// FastList.newInstance();
    ...
    public static List<GenericValue> getCurrentProductCategories(GenericValue product) {
        ...
        List<GenericValue> categories =new ArrayList<GenericValue>(); //FastList.newInstance();
        try {
    ...
    public static GenericValue getParentProductWithoutCache(String productId, Delegator delegator) {
        GenericValue _parentProduct = null;
        if (productId == null) {
            Debug.logWarning("Bad product id", module);
        }
        ...
        try {
            List<GenericValue> virtualProductAssocs = delegator.findByAnd("ProductAssoc", UtilMisc.toMap("productIdTo", productId, "productAssocTypeId", "PRODUCT_VARIANT"), UtilMisc.toList("-fromDate"));
            virtualProductAssocs = EntityUtil.filterByDate(virtualProductAssocs);
            if (UtilValidate.isEmpty(virtualProductAssocs)) {
                //okay, not a variant, try a UNIQUE_ITEM
                virtualProductAssocs = delegator.findByAnd("ProductAssoc", UtilMisc.toMap("productIdTo", productId, "productAssocTypeId", "UNIQUE_ITEM"), UtilMisc.toList("-fromDate"));
                virtualProductAssocs = EntityUtil.filterByDate(virtualProductAssocs);
            }
            if (UtilValidate.isNotEmpty(virtualProductAssocs)) {
                //found one, set this first as the parent product
                GenericValue productAssoc = EntityUtil.getFirst(virtualProductAssocs);
                _parentProduct = productAssoc.getRelatedOne("MainProduct");
            }
        } catch (GenericEntityException e) {
            throw new RuntimeException("Entity Engine error getting Parent Product (" + e.getMessage() + ")");
        }
...
//              Debug.log("=====product found:" + productId + " and features: " + selectedFeatures);
    ...
    public static BigDecimal totalAvailableATP(Delegator delegator, LocalDispatcher dispatcher, GenericValue productStore, String productId){
    	return totalAvailableATP(delegator, dispatcher, productStore.getString("inventoryFacilityId"), productId ,null);
    }
public static BigDecimal totalAvailableATP(Delegator delegator, LocalDispatcher dispatcher, String inventoryFacilityId, String productId, String inventoryItemId){
		...
		Map inventoryAvailable = totalInventory(delegator, dispatcher, inventoryFacilityId, productId, inventoryItemId);
	...
	public static BigDecimal totalAvailableQOH(Delegator delegator, LocalDispatcher dispatcher, String inventoryFacilityId, String productId, String inventoryItemId){
	...
	if(UtilValidate.isNotEmpty(inventoryAvailable))
		quantityOnHandTotal = (BigDecimal)inventoryAvailable.get("quantityOnHandTotal");
	
	return quantityOnHandTotal;
}
	
	public static Map totalInventory(Delegator delegator, LocalDispatcher dispatcher, String inventoryFacilityId, String productId, String inventoryItemId){
    	...
    	{
    		try {
    			Map condition = UtilMisc.toMap("productId",productId);
    			if(UtilValidate.isNotEmpty(inventoryItemId))
    				condition.put("inventoryItemId", inventoryItemId);
    			
				inventoryAvailable = dispatcher.runSync("getProductInventoryAvailable", condition);
			} catch (GenericServiceException e) {
    	...
    	{
    		Map condition = UtilMisc.toMap("productId",product.getString("productId"),"facilityId",inventoryFacilityId);
			if(UtilValidate.isNotEmpty(inventoryItemId))
				condition.put("inventoryItemId", inventoryItemId);
			
    		try {
				inventoryAvailable = dispatcher.runSync("getInventoryAvailableByFacility", condition);
			} catch (GenericServiceException e) {
				Debug.logError(e, "Problems getting inventory available by facility id "+inventoryFacilityId+" for product "+product.getString("productId"), module);
			}
    	}
    	try {
	...
	public static List<GenericValue> getProductList(Delegator delegator, String categoryId, String productId, String brandName){
		
		List condition = new ArrayList();
		if(UtilValidate.isNotEmpty(categoryId))
			condition.add(EntityCondition.makeCondition("primaryProductCategoryId",EntityOperator.EQUALS,categoryId.trim()));
		
		if(UtilValidate.isNotEmpty(productId))
			condition.add(EntityCondition.makeCondition("productId",EntityOperator.EQUALS,productId.trim()));
		if(UtilValidate.isNotEmpty(brandName))
		{
			brandName = "%"+brandName+ "%";
			condition.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("brandName"),EntityOperator.LIKE,EntityFunction.UPPER(brandName.trim().toUpperCase())));
		}
		if(condition.size() == 0)
			return null;
		try {
			return delegator.findList("Product", 
