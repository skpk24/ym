...
public class ProductSearchEvents {
   ...
   public static String searchAddToCategory(HttpServletRequest request, HttpServletResponse response) {
    ...
    public static String searchExportProductList(HttpServletRequest request, HttpServletResponse response) {
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        String errMsg = null;
        List<Map<String, Object>> productExportList =new ArrayList<Map<String,Object>>();// FastList.newInstance();
        ...
        try {
            ...
            try {
                ...
                GenericValue searchResultView = null;
                while ((searchResultView = eli.next()) != null) {
                    Map<String, Object> productMap = new HashMap<String, Object>(); //FastMap.newInstance();
                    String productId = searchResultView.getString("mainProductId");
    ...
    public static String createProductBarcode(HttpServletRequest request, HttpServletResponse response) {
    	Delegator delegator = (Delegator) request.getAttribute("delegator");
        ...
        GenericValue prodid = null;
        if (productId != null && productBarcode != null) {
            try {
            	if(productBarcode.contains(" ")){
            		productBarcode = productBarcode.replaceAll("\\s","");
            	}
            	prodid = delegator.findByPrimaryKey("ProductBarcode", UtilMisc.toMap("productId", productId, "barcode", productBarcode));
            	if(prodid == null){
            		try{
            		GenericValue barcodeDettail = delegator.makeValue("ProductBarcode");
            		barcodeDettail.put("productId",productId);
            		barcodeDettail.put("barcode",productBarcode);
             		barcodeDettail.create();
           		 	msgList.add("Barcode Created Successfully : "+productBarcode);
            		...
            		///////////////////
            		 msgList.add("Entered Barcode Already Exists : "+productBarcode);
            		//////////////////////////////
             	}
            }catch (GenericEntityException e) {
	            request.setAttribute("_ERROR_MESSAGE_", e.getMessage());
...
public static String updateProductBarcode(HttpServletRequest request, HttpServletResponse response) {
	Delegator delegator = (Delegator) request.getAttribute("delegator");
	List<String> msgList=new ArrayList<String>();
    String productBarcodeOld = request.getParameter("productBarcodeOld");
    String productBarcode = request.getParameter("productBarcode");
    
      String productId = request.getParameter("productId");
    GenericValue prodid = null;
    if (productId != null && productBarcodeOld != null && productBarcode!=null ) {
        try {
        	if(productBarcodeOld.contains(" ")){
        		productBarcodeOld = productBarcodeOld.replaceAll("\\s","");
        	}
        	prodid = delegator.findByPrimaryKey("ProductBarcode", UtilMisc.toMap("productId", productId, "barcode", productBarcodeOld));
        	GenericValue oldprodid = delegator.findByPrimaryKey("ProductBarcode", UtilMisc.toMap("productId", productId, "barcode", productBarcode));
        	if(oldprodid==null){
         	if(prodid != null ){
        		try{
        			prodid.remove();
         			prodid.put("productId", productId);
        			prodid.put("barcode", productBarcode);
        			prodid.create();
        			
       		 	msgList.add("Barcode Updated Successfully : "+productBarcode);
        		...
        		msgList.add("Barcode already Exists: "+productBarcode);
        	}
        }catch (GenericEntityException e) {
            request.setAttribute("_ERROR_MESSAGE_", e.getMessage());
            return "error";
...
public static String deleteProductBarcode(HttpServletRequest request, HttpServletResponse response) {
	Delegator delegator = (Delegator) request.getAttribute("delegator");
	List<String> msgList=new ArrayList<String>();
     String productBarcode = request.getParameter("productBarcode");
    ...
    if (productId != null &&  productBarcode!=null ) {
        try {
        	...
        	if(prodid != null){
        		try{
        			prodid.remove();
       		 	msgList.add("Barcode Deleted Successfully : "+productBarcode);
       		 	
        		}catch (GenericEntityException e) {
        			msgList.add("Error in deleteing Barcode : "+productBarcode);
    	            request.setAttribute("_ERROR_MESSAGE_", e.getMessage());
    	            return "error";
    	        }
        	}
