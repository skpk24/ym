    ...
    <div class="screenlet-body">
        ...
        <#else>
              ...
              <#list productCategoryMembers as productCategoryMember>
                ...
                <#if productCategoryMember.thruDate?exists && nowTimestamp.after(productCategoryMember.getTimestamp("thruDate"))><#assign hasExpired = true></#if>
                  <tr valign="middle"<#if rowClass == "1"> class="alternate-row"</#if>>
                    ...
                    <td <#if hasntStarted> style="color: red;"</#if>><#if productCategoryMember.fromDate?has_content>${(productCategoryMember.fromDate)?string("dd-MM-yyyy")?if_exists}</#if></td>
                    <td align="center">
           ...
           <form name="deleteProductFromCategory_o_${rowCount}" method="post" action="<@ofbizUrl>removeCategoryProductMember</@ofbizUrl>">
              ...
              <input type="hidden" name="activeOnly" value="${activeOnly.toString()}"/>
              <input type="hidden" name="primaryProductCategoryId" value="${(productCategoryMember.productCategoryId)?if_exists}"/>
           </form>
           <#assign rowCount = rowCount + 1>
           </#list>        
      </#if>
