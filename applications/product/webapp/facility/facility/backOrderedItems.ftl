...
<#macro displayDate date="" format="DATE_TIME" class="tabletext" style="" default="" compareTo="" highlightAfter=false highlightBefore=false highlightStyle="color:red;">
  ...
  <span class="${class}" style="${applyStyle}"><#if date?has_content>${date}<#else>${default}</#if></span>
</#macro>
...
<#macro pagination viewIndex viewSize currentResultSize requestName totalResultSize=-1 extraParameters="">
    <#if ( ! totalResultSize?has_content ) || (totalResultSize?has_content && totalResultSize != 0)>
        ...
        <div class="pagination">
            <#if beginIndex &gt; 1>
                <span class="paginationPrevious"><a href="<@ofbizUrl>${requestName}?VIEW_INDEX=${beginIndex - viewSize}&amp;VIEW_SIZE=${viewSize}${extraParameters?html}&amp;facilityId=${parameters.facilityId}</@ofbizUrl>">${uiLabelMap.CommonPrevious}</a></span>
            </#if>
            <#--<span class="paginationText"><@paginationText/></span>-->
            <#if endIndex &lt; totalResultSize>
                <span class="paginationNext"><a href="<@ofbizUrl>${requestName}?VIEW_INDEX=${beginIndex + viewSize}&amp;VIEW_SIZE=${viewSize}${extraParameters?html}&amp;facilityId=${parameters.facilityId}</@ofbizUrl>">${uiLabelMap.CommonNext}</a></span>
            </#if>
        </div>
    </#if>
</#macro>
...
<div class="screenlet">
    <div class="screenlet-title-bar">
    	<div class="h3">Search Option</div>
    </div>
    <div class="screenlet-body">
		<form method="get" target="" name="findBackOrderedItems">
		<table class="twoColumnForm">
		  <tr>
		    <td class="titleCell"><span class="tableheadtext">${uiLabelMap.ProductOrderStatus}</span></td>
		    <td>
		      <select name="statusId" class="inputBox">
		        ...
		        <option value="ORDER_HOLD" <#if parameters.statusId?default("") == "ORDER_HOLD">selected</#if>>${uiLabelMap.CommonOnHold}</option>
		        <option value="ANY" <#if parameters.statusId?default("") == "ANY">selected</#if>>${uiLabelMap.CommonAny}</option>
		      </select>
		    </td>
		  </tr>
		   <#-- Remove That if not required,Previous code-->
		   <#--<@inputAutoCompleteProductRow name="productId" title=uiLabelMap.ProductProductId default=parameters.productId />-->
	    
	    <#-- new Code Inserted  Remove That if not required-->
	    <td class="titleCell"><span class="tableheadtext">${uiLabelMap.ProductProductId}</span></td>	  
		  <td align='left'>
              <input type='text' name='productId'/>
                  <a href="javascript:call_fieldlookup2(document.findBackOrderedItems.productId,'LookupProduct');">
                    <img src='/images/fieldlookup.gif' width='15' height='14' border='0' alt="${uiLabelMap.CommonClickHereForFieldLookup}"/>
                  </a>
        </td>
		    <#-- new Code End  Remove That if not required-->
...
<div class="screenlet">
    ...
    <div class="screenlet-body">
		<table class="basic-table" cellspacing="0" >
		    <tr class="header-row">
		        <td><b>${uiLabelMap.ProductReservedOn}</b></td>
		        <td><b>${uiLabelMap.ProductProductId}</b></td>
		        <td><b>${uiLabelMap.OrderOrderDate}</b></td>
		        <td><b>${uiLabelMap.OrderOrder}</b></td>
		        <td><b>${uiLabelMap.CommonStatus}</b></td>
		        <td><b>${uiLabelMap.OrderShipVia}</b></td>
		        <td><b>${uiLabelMap.OrderQtyOrdered}</b></td>
		        <td><b>${uiLabelMap.OrderQtyBackOrdered}</b></td>
		        <td><b>${uiLabelMap.FormFieldTitle_promisedDatetime}</b></td>        
		    </tr>
		    <#list backOrderedItems?if_exists as item>
			    ...
			    <tr class="${tableRowClass(item_index)}">
			        ...
			        <@displayLinkCell  href="/ordermgr/control/orderview?orderId=${item.orderId}" text=item.orderId/>
