...
<div class="screenlet">
    ...
    <div class="screenlet-body">
      <table class="basic-table" cellspacing='1'>
        <tr>
          <td align="left" valign="top" width="15%" class="orderlabeltext">${uiLabelMap.CommonName}</td>
           <td>
            <div class="ordersubtext" text-align="left">
              ...
              <#if partyId?exists>
                &nbsp;(<a href="${customerDetailLink}${partyId}" target="partymgr" class="buttontext1">${partyId}</a>)
                </br>
                <#if orderHeader.salesChannelEnumId != "POS_SALES_CHANNEL">
                <div>
                   <a href="<@ofbizUrl>/orderentry?partyId=${partyId}&amp;orderTypeId=${orderHeader.orderTypeId}</@ofbizUrl>" class="buttontext1">${uiLabelMap.OrderNewOrder}</a>
                   <a href="javascript:document.searchOtherOrders.submit()" class="buttontext1">${uiLabelMap.OrderOtherOrders}</a>
                </div>
        ...
        <#list orderContactMechValueMaps as orderContactMechValueMap>
          ...
          <tr>
            <td align="left" valign="top" width="5%">
              <span class="orderlabeltext">${contactMechPurpose.get("description",locale)}</span>
            </td>
            <td valign="top" width="80%">
              <#if contactMech.contactMechTypeId == "POSTAL_ADDRESS">
                ...
                <#if postalAddress?has_content>
                  <div>
                    <#if postalAddress.toName?has_content><span class="orderlabeltext">${uiLabelMap.CommonTo}</span> ${postalAddress.toName}<br/></#if>
                    <#if postalAddress.attnName?has_content><span class="orderlabeltext">${uiLabelMap.CommonAttn}</span> ${postalAddress.attnName}<br/></#if>
                   <span class="ordersubtext"> ${postalAddress.address1}</span><br/>
                    <span class="ordersubtext"><#if postalAddress.address2?has_content>${postalAddress.address2}</span><br/></#if>
                    <span class="ordersubtext">${postalAddress.city?if_exists}</span>
                    <#if postalAddress.stateProvinceGeoId?has_content>, <span class="ordersubtext">${postalAddress.stateProvinceGeoId}</span> </#if>
                   <span class="ordersubtext"> ${postalAddress.postalCode?if_exists}</span><br/>
                   <span class="ordersubtext"> ${postalAddress.countryGeoId?if_exists}</span><br/>
                    <#if !postalAddress.countryGeoId?exists || postalAddress.countryGeoId == "USA">
                      <#assign addr1 = postalAddress.address1?if_exists>
                      <#if (addr1.indexOf(" ") > 0)>
                        <#assign addressNum = addr1.substring(0, addr1.indexOf(" "))>
                        <#assign addressOther = addr1.substring(addr1.indexOf(" ")+1)>
                        <a target="_blank" href="${uiLabelMap.CommonLookupWhitepagesAddressLink}" class="buttontext1">${uiLabelMap.CommonLookupWhitepages}</a>
                      </#if>
                    </#if>
                  </div>
              ...
              <#elseif contactMech.contactMechTypeId == "TELECOM_NUMBER">
                ...
                <div>
                  ...
                  <#if !telecomNumber.countryCode?exists || telecomNumber.countryCode == "011" || telecomNumber.countryCode == "1">
                    <a target="_blank" href="${uiLabelMap.CommonLookupAnywhoLink}" class="buttontext1">${uiLabelMap.CommonLookupAnywho}</a>
                   <a target="_blank" href="${uiLabelMap.CommonLookupWhitepagesTelNumberLink}" class="buttontext1">${uiLabelMap.CommonLookupWhitepages}</a>
                  </#if>
                </div>
              <#elseif contactMech.contactMechTypeId == "EMAIL_ADDRESS">
                <div>
                  ...
                  <#if security.hasEntityPermission("ORDERMGR", "_SEND_CONFIRMATION", session)>
                     (<a href="<@ofbizUrl>confirmationmailedit?orderId=${orderId}&amp;partyId=${partyId}&amp;sendTo=${contactMech.infoString}</@ofbizUrl>" class="buttontext1">${uiLabelMap.OrderSendConfirmationEmail}</a>)
                  <#else>
                     <a href="mailto:${contactMech.infoString}" class="buttontext1">(${uiLabelMap.OrderSendEmail})</a>
                  </#if>
                </div>
              <#elseif contactMech.contactMechTypeId == "WEB_ADDRESS">
                <div>
                  ...
                  <a target="_blank" href="${openString}" class="buttontext1">(open&nbsp;page&nbsp;in&nbsp;new&nbsp;window)</a>
                </div>
              <#else>
                <div>
