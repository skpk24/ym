...
if (orderId) {
	org.ofbiz.order.order.OrderServices.reserveAllForPacking(request,response);
        ...
        if ("ORDER_APPROVED".equals(orderHeader.statusId)) {
            ...
            if (!shipment) {
                ...
                if (!picklistBinId) {
                    ...
                    //List items = org.ofbiz.shipment.packing.PackingServices.calculateQuantityForPacking(packSession);
                    //packSession.addAllItemInfos(items);
...
context.assignedVechilesToDeliveryBoy = org.ofbiz.order.DeliveryBoyVechileManagement.vehicleWithThreshold(delegator);

orderReadHelper = context.orderReadHelper;
packingSession = context.packingSession;
