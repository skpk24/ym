...
salechannel = request.getAttribute("salechannel");
...
while ( iterator.hasNext() ){
	      GenericValue stemp =(GenericValue) iterator.next();
	     // print("this is my status id"+stemp);
	     print("this is my status id??????????????"+stemp.getString("statusId"));
	 	    statId=stemp.getString("statusId")
	 	    print"&&&&&&&&&&&the value of&&&&&&&&&&"+statId
	...
	print"%%%%%%%%%%%%%%%%%%%%%the value of sulords is%%%^******************"+sulords;
	
	print"******%%%%%%*****this value of penords is*******%%%%%%********"+penords;
	
	print"\n\n"
	context.sulords=sulords;
	//  globalContext = context.get("globalContext");
	//  globalContext.put("sulords", penords);
...
//context.terminals = delegator.findAll("PosTerminal");
