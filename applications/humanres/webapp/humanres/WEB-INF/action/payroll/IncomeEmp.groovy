...
import java.util.*;
  List mainAndExprsCon =[];
        mainAndExprsCon.add(EntityCondition.makeCondition("parentTypeId", EntityOperator.EQUALS, "PAYROL_EARN_HOURS"));
...
for(int i=0;i<ItemTypes.size();i++)
{
  ...
  for(int i=0;i<ItemTypes.size();i++)
          ...
          {
          totalincome=Double.parseDouble(costValue[i])+totalincome;
          }
      ...
      empsal.put("attrType","PAYROL_EARN_HOURS");
  ...
  for(int i=0;i<ItemTypes.size();i++)
     ...
     party.set("grossPay",totalincome);
     if(party.get("netDeduction")!=null)
   	totaldeduction =party.get("netDeduction");
 	 netPay= totalincome-totaldeduction;
    party.set("netPay",netPay);
    party.store();
