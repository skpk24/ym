...
public class OrderManagerEvents {
    ...
    public static String receiveOfflinePayment(HttpServletRequest request, String orderId, String amountStrNew, String paymentReferenceNew) {
        HttpSession session = request.getSession();
        LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        GenericValue userLogin = (GenericValue) session.getAttribute("userLogin");
        ...
        boolean fulfillOrderFlag = false;
        ...
        try {
        	EntityExpr ee = null;
        	if(UtilValidate.isNotEmpty(paymentReferenceNew))
        		ee = EntityCondition.makeCondition("paymentMethodTypeId", EntityOperator.EQUALS, paymentReferenceNew);
        	else
        		ee = EntityCondition.makeCondition("paymentMethodTypeId", EntityOperator.NOT_EQUAL, "EXT_OFFLINE");
      ...
      //  List<GenericValue> toBeStored = FastList.newInstance();
        ...
        while (pmti.hasNext()) {
            ...
            if(UtilValidate.isEmpty(amountStr))amountStr = amountStrNew;
            if(UtilValidate.isEmpty(paymentReference))paymentReference = paymentReferenceNew;
            
            if(UtilValidate.isNotEmpty(amountStrNew) && UtilValidate.isNotEmpty(paymentReferenceNew))
            	fulfillOrderFlag = true;
            ...
            if (!UtilValidate.isEmpty(amountStr)) {
                ...
                if (paymentTypeAmount.compareTo(BigDecimal.ZERO) > 0) {
                    ...
                    //new code just checking the for credit amounts....
                    
                    //List exprs = FastList.newInstance();
                    List exprs = new ArrayList();
                    exprs.add(EntityCondition.makeCondition("orderId", EntityOperator.EQUALS, orderId));
                    exprs.add(EntityCondition.makeCondition("paymentMethodTypeId", EntityOperator.EQUALS, "EXT_COD"));
        	        exprs.add(EntityCondition.makeCondition("isCredit", EntityOperator.EQUALS, "Yes"));
                    List orderBy = UtilMisc.toList("-createdStamp");
                    
                    List <GenericValue>orderpreffList = null;
                    try {
                        orderpreffList = delegator.findList("OrderPaymentPreference", EntityCondition.makeCondition(exprs, EntityOperator.AND), null, orderBy, null, false);
                    } catch (GenericEntityException e) {
                        Debug.logError(e, module);
                    }  
                    if(orderpreffList!=null){
                    	paymentPreference.set("isCredit", "Done");
                    	 GenericValue OH=null;
                 	      try {
							OH=delegator.findOne("OrderHeader", false, UtilMisc.toMap("orderId", orderId));
						} catch (GenericEntityException e) {
							e.printStackTrace();
						}
               	    
						if(OH!=null)
               	    	    paymentPreference.set("billingPartyId",OH.getString("billToPartyId"));
                       }
        ...
        if (okayToApprove && !fulfillOrderFlag) {
