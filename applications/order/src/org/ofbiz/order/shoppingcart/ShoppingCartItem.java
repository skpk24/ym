...
public class ShoppingCartItem implements java.io.Serializable {
   ...
   // private List<GenericValue> itemAdjustments = FastList.newInstance();
    private List<GenericValue> itemAdjustments = new ArrayList<GenericValue>();
    ...
    private String parentProductCategoryId = null; // parentCategoryId
    
    private long cartSeqId = 1;
    
    private String recipientMobileNum = null;
    private String recipientEmailId = null;
             ...
             //   Map<String, Object> priceContext = FastMap.newInstance();
                ...
                if (cart.getOrderType().equals("PURCHASE_ORDER")) {
                    ...
                    if (Boolean.FALSE.equals(validPriceFound)) {
                        throw new CartItemModifyException("Could not find a valid price for the product with ID [" + this.getProductId() + "], not adding to cart.        "+priceResult);
                    }
   ...
   public List<GenericValue> getFeaturesForSupplier(LocalDispatcher dispatcher,String partyId) {
    ...
    public BigDecimal getmodifyAmount() {
        ...
        if (this.listPrice!= null) 
        	curListPrice=this.listPrice;
        else
        	curListPrice = this.getListPrice();
        
        if(UtilValidate.isNotEmpty(curListPrice) && UtilValidate.isNotEmpty(curDisplayPrice)&& curListPrice.compareTo(curDisplayPrice)==1 )
        return (curListPrice.subtract(curDisplayPrice)).setScale(2, RoundingMode.CEILING);
        else
...
//        if (orderItemAttributes == null) orderItemAttributes = FastMap.newInstance();
	...
	public long getCartSeqId() {
		return cartSeqId;
	}

	public void setCartSeqId(long cartSeqId) {
		this.cartSeqId = cartSeqId;
	}

	public String getRecipientMobileNum() {
		return recipientMobileNum;
	}

	public void setRecipientMobileNum(String recipientMobileNum) {
		this.recipientMobileNum = recipientMobileNum;
	}
