...
public class ShoppingCart implements Iterable<ShoppingCartItem>, Serializable {
    ...
    private BigDecimal inviteAccountAmt = BigDecimal.ZERO;
   ...
   // private List<GenericValue> adjustments = FastList.newInstance();
    private List<GenericValue> adjustments = new ArrayList<GenericValue>();
    // OrderTerms
    ...
    private List<ShoppingCartItem> cartLines = new ArrayList<ShoppingCartItem>();
    ...
    private List<CartPaymentInfo> paymentInfo = new ArrayList<CartPaymentInfo>();
    private List<CartShipInfo> shipInfo = new ArrayList<CartShipInfo>();
...
*/
    private List<String> internalOrderNotes = new ArrayList<String>(); // internal notes
    private List<String> orderNotes = new ArrayList<String>(); // public notes (printed on documents etc.)
  ...
  //  private List<ProductPromoUseInfo> productPromoUseInfoList = FastList.newInstance();
    private List<ProductPromoUseInfo> productPromoUseInfoList = new ArrayList<ProductPromoUseInfo>();
    ...
    public ShoppingCart(ShoppingCart cart) {
        ...
        this.inviteAccountAmt = cart.getInviteFriendAccount();
    ...
    public int addOrIncreaseItem(String productId, BigDecimal selectedAmount, BigDecimal quantity, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons,
            ...
            ProductConfigWrapper configWrapper, String itemType, String itemGroupNumber, String parentProductId, String recipientMobileNum, String recipientEmailId, String recipientName, String recipientMessage, LocalDispatcher dispatcher) throws CartItemModifyException, ItemNotFoundException {
       ...
       return addOrIncreaseItem(productId,selectedAmount,quantity,reservStart,reservLength,reservPersons,
                ...
                configWrapper,itemType,itemGroupNumber,parentProductId,recipientMobileNum,recipientEmailId, recipientName, recipientMessage, dispatcher);
    }
    ...
    public int addOrIncreaseItem(String productId, BigDecimal selectedAmount, BigDecimal quantity, Timestamp reservStart, BigDecimal reservLength, BigDecimal reservPersons,
        ...
        if (getOrderType().equals("PURCHASE_ORDER")) {
            ...
            */
            ShoppingCartItem item = ShoppingCartItem.makeItem(Integer.valueOf(0), productId, selectedAmount, quantity, null,
                    reservStart, reservLength, reservPersons, accommodationMapId, accommodationSpotId, shipBeforeDate, shipAfterDate,
                    features, attributes, prodCatalogId, configWrapper, itemType, itemGroup, dispatcher,
                    this, Boolean.TRUE, Boolean.TRUE, parentProductId, Boolean.FALSE, Boolean.FALSE);
        	String url = ShoppingCartEvents.getProductIndexUrl(delegator, dispatcher, item.getProductId(), item.getProductId());
        	item.setProductUrl(url);

            
            if(UtilValidate.isNotEmpty(recipientMobileNum) && UtilValidate.isNotEmpty(item) )
              item.setRecipientMobileNum(recipientMobileNum);
            
            if(UtilValidate.isNotEmpty(recipientEmailId) && UtilValidate.isNotEmpty(item) )
              item.setRecipientEmailId(recipientEmailId);
            
            if(UtilValidate.isNotEmpty(recipientName) && UtilValidate.isNotEmpty(item) )
                item.setRecipientName(recipientName);
            
            if(UtilValidate.isNotEmpty(recipientMessage) && UtilValidate.isNotEmpty(item) )
                item.setRecipientMessage(recipientMessage);
...
//            nextItemSeq++;
    ...
    public List<ShoppingCartItem> findAllCartItems(String productId, String groupNumber) {
       ...
       // List<ShoppingCartItem> itemsToReturn = FastList.newInstance();
        List<ShoppingCartItem> itemsToReturn = new ArrayList<ShoppingCartItem>();
        // Check for existing cart item.
    ...
    public void removeExtraItems(List<ShoppingCartItem> multipleItems, LocalDispatcher dispatcher, int maxItems) throws CartItemModifyException {
        ...
        List<ShoppingCartItem> localList = new ArrayList<ShoppingCartItem>();
...
//    protected String billFromVendorPartyId = null;
  //  protected String shipFromVendorPartyId = null;
    ...
    public void clear() {
        ...
        this.inviteAccountAmt = BigDecimal.ZERO;
    ...
    public List<GenericValue> getPaymentMethods() {
      //  List<GenericValue> methods = FastList.newInstance();
        List<GenericValue> methods = new ArrayList<GenericValue>();
    ...
    //set the invite ffriend ref amount
    public void setInviteFriendAccount(BigDecimal amount) {
       
        this.inviteAccountAmt = amount;
    }
    ...
    /** Returns the inviteFriend. */
    public BigDecimal getInviteFriendAccount() {
        return this.inviteAccountAmt;
    }
    ...
    public void removeAdjustmentByType(String orderAdjustmentTypeId) {
      ...
      //  List<List<GenericValue>> adjsLists = FastList.newInstance();
        List<List<GenericValue>> adjsLists = new ArrayList<List<GenericValue>>();
        ...
        synchronized (cartLines) {
         //   List<GenericValue> result = FastList.newInstance();
            ...
            for (ShoppingCartItem item : cartLines) {
                ...
                if(UtilValidate.isEmpty(item.getProductCategoryId()))
            	{
	                ...
	                if(UtilValidate.isNotEmpty(product1))
	                	...
	                	orderItem.set("productCategoryId", productCategoryId);
                ...
                orderItem.set("recipientEmailId", item.getRecipientEmailId());
                orderItem.set("recipientMobileNum", item.getRecipientMobileNum());
                orderItem.set("recipientName", item.getRecipientName());
                orderItem.set("recipientMessage", item.getRecipientMessage());
                ...
                item.setParentProductCategoryId(org.ofbiz.product.category.CategoryWorker.getProdCatalogOrCategoryId(delegator,productId));
                orderItem.set("parentProductCategoryId", item.getParentProductCategoryId());
    ...
    public List<GenericValue> makeAllAdjustments() {
     //   List<GenericValue> allAdjs = FastList.newInstance();
        List<GenericValue> allAdjs = new ArrayList<GenericValue>();
    ...
    public List<GenericValue> makeAllQuoteAdjustments() {
      //  List<GenericValue> quoteAdjs = FastList.newInstance();
        List<GenericValue> quoteAdjs = new ArrayList<GenericValue>();
    ...
    public List<GenericValue> makeProductPromoUses() {
       // List<GenericValue> productPromoUses = FastList.newInstance();
        List<GenericValue> productPromoUses = new ArrayList<GenericValue>();
        String partyId = this.getPartyId();
        int sequenceValue = 0;
        for (ProductPromoUseInfo productPromoUseInfo: this.productPromoUseInfoList) {
    ...
    public List<GenericValue> makeAllOrderAttributes(String orderId, int mode) {
       ...
       // List<GenericValue> allOrderAttributes = FastList.newInstance();
        List<GenericValue> allOrderAttributes = new ArrayList<GenericValue>();
    ...
    public static class CartShipInfo implements Serializable {
      ...
      //  public List<GenericValue> shipTaxAdj = FastList.newInstance();
        public List<GenericValue> shipTaxAdj = new ArrayList<GenericValue>();
