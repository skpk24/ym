...
import org.ofbiz.order.ZoneManagement;
...
public class CheckOutEvents {
    ...
    public static String setCheckOutPages(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
        if ("error".equals(CheckOutEvents.cartNotEmpty(request, response)) == true) {
            //return "error";
        	return "home";
        }
        
        Map qtyMap = ShoppingCartEvents.totalQtyAddedInCartList(request);
        ...
        boolean flag = ShoppingCartEvents.canBuyGiftCard(request);
        if(flag)
        {
        ...
        if ("shippingaddress".equals(curPage) == true) {
          ...
          if(UtilValidate.isEmpty(request.getParameter("shipping_method")))
        	  shippingMethod="NO_SHIPPING@Company";
        	//String shippingMethod ="LOCAL_DELIVERY";
        ...
        Date d = null;
				try {
        		 ...
        		 String contactMechId=request.getParameter("mechId");
        	String slot= data2[0];
            ...
            Map<String, ? extends Object> slotResult = new HashMap();
            for (int shipGroupIndex = 0; shipGroupIndex < cart.getShipGroupSize(); shipGroupIndex++) {
            ...
            if (!(callResult.get(ModelService.RESPONSE_MESSAGE).equals(ModelService.RESPOND_ERROR))) {
                // No errors so push the user onto the next page
                curPage = "reviewBefore";
                }
            
         
            //for slot modification
            slotResult=checkOutHelper.setSlotDelivery(slot,deliveryDate);
            
           // GenericValue userLogin = (GenericValue) session.getAttribute("userLogin");
            HashMap<String, Object> orderSlotMap = new HashMap();
            orderSlotMap.put("slotId", delegator.getNextSeqId("OrderSlot"));
            orderSlotMap.put("slotType", slot);
            orderSlotMap.put("slotStatus", "SLOT_REQUESTED");
            orderSlotMap.put("contactMechId",contactMechId);
            orderSlotMap.put("deliveryDate", deliveryDate);
            orderSlotMap.put("partyId", userLogin.get("partyId"));
            orderSlotMap.put("orderId", null);
            GenericValue orderSlot = delegator.makeValue("OrderSlot", orderSlotMap);
            orderSlot.create();
          
            
            ServiceUtil.getMessages(request, slotResult, null);
            if (!(slotResult.get(ModelService.RESPONSE_MESSAGE).equals(ModelService.RESPOND_ERROR))) {
                // No errors so push the user onto the next page
                //curPage = "payment";
            	curPage = "reviewBefore";
                }
            
        }else if("reviewBefore".equals(curPage) == true){
        	curPage = "payment";
         ...
         for(Object payment : cart.getPaymentInfo()){
            	if(UtilValidate.isNotEmpty(payment) && payment instanceof CartPaymentInfo)
            	{
            		if("EXT_NETBANKING".equals(((CartPaymentInfo)payment).paymentMethodTypeId))
            			((CartPaymentInfo)payment).bankId = request.getParameter("citrusBankId");
            		else
            			((CartPaymentInfo)payment).bankId = null;
            	}
            }
            ServiceUtil.getMessages(request, callResult, null);
            ...
            if(finalValidationOnCreateOrder(request,response)){
             request.setAttribute("_ERROR_MESSAGE_", "Cart subtotal is less than 500");
            	return "showcart";
            }
        ...
        if (paymentMethods != null) {
            for (int i = 0; i < paymentMethods.length; i++) {
               // Map<String, Object> paymentMethodInfo = FastMap.newInstance();
                ...
                // for invite ref for youmart
              if(paymentMethods[i].equals("EXT_INVITE") && UtilValidate.isNotEmpty(amountStr))
                {
                	BigDecimal tem=new BigDecimal(amountStr); 
                	amountStr=((tem.multiply(cart.getItemTotal())).divide(new BigDecimal(100))).toString();
               ...
               if(paymentMethods[i].equals("EXT_INVITE") && UtilValidate.isNotEmpty(amount))
                selectedPaymentMethods.put(paymentMethods[i], paymentMethodInfo);
               if(!paymentMethods[i].equals("EXT_INVITE"))
            	   selectedPaymentMethods.put(paymentMethods[i], paymentMethodInfo);  
    ...
    public static String setCheckOutOptions(HttpServletRequest request, HttpServletResponse response) {
     ...
     // String shippingMethod=request.getParameter("shipping_method");
    	...
    	if(!UtilValidate.isEmpty(request.getParameter("dayOption")))
    	{
    	String data1=request.getParameter("dayOption");
       ...
       // GenericValue userLogin = (GenericValue) session.getAttribute("userLogin");
        ...
        orderSlotMap.put("partyId", userLogin.get("partyId"));
        orderSlotMap.put("orderId", null);
         orderSlotMap.put("contactMechId",shippingContactMechId);
         GenericValue orderSlot = delegator.makeValue("OrderSlot", orderSlotMap);
        try {
			orderSlot.create();
    ...
    public static boolean  finalValidationOnCreateOrder(HttpServletRequest request, HttpServletResponse response) {
    	  HttpSession session = request.getSession();
          ShoppingCart cart = ShoppingCartEvents.getCartObject(request);
         if(org.ofbiz.order.shoppingcart.ShoppingCartEvents.isCartContainsGiftCard(cart)){
        	 if(cart.getPaymentMethodTypeIds().contains("EXT_COD")){
         ...
         BigDecimal grandTotal = cart.getDisplayGrandTotal();
         BigDecimal adjustment = BigDecimal.ZERO;
         List<GenericValue> cartAdjustments = cart.getAdjustments();
         if(UtilValidate.isNotEmpty(cartAdjustments)){
        	 for(GenericValue cartAdjustment : cartAdjustments)
        		 adjustment= adjustment.add(org.ofbiz.order.order.OrderReadHelper.calcOrderAdjustment(cartAdjustment, cart.getSubTotal()));
        	 
        	 grandTotal = cart.getDisplaySubTotal();
         }
         
         adjustment = adjustment.negate();
         if(grandTotal.intValue() < 500 && adjustment.intValue() < 500){
        	 return true;
         }
...
//    	System.out.println("\n first request");
        ...
        String slot=cart.getslot();
        Timestamp deliveryDate=cart.getDeliveryDate();
        ...
        if (callResult != null) {
            ...
            if (callResult.get(ModelService.RESPONSE_MESSAGE).equals(ModelService.RESPOND_SUCCESS)) {
                ...
                //For creating zones for order
                createOrderZone(delegator,orderId);
    ...
    public static void createOrderZone(Delegator delegator, String orderId){
		...
		for(GenericValue shipGroup : shipGroups)
		{
			...
			try {
				shippingAddress = shipGroup.getRelatedOne("PostalAddress");
			} catch (GenericEntityException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			if(UtilValidate.isNotEmpty(shippingAddress) && UtilValidate.isNotEmpty(shippingAddress.getString("postalCode")))
			{
				GenericValue pinDetails = ZoneManagement.getPinCodeDetails(delegator, shippingAddress.getString("postalCode"));
				if(UtilValidate.isNotEmpty(pinDetails))
				{
					String zoneName = pinDetails.getString("zoneName");
					String pinCode = pinDetails.getString("pinCode");
					String zoneGroupId = pinDetails.getString("zoneGroupId");
					String zoneGroupName = pinDetails.getString("zoneGroupName");
					
					GenericValue orderZone = delegator.makeValue("OrderZone");
					
					orderZone.put("orderZoneId", delegator.getNextSeqId("OrderZone"));
					orderZone.put("zoneName", zoneName);
					orderZone.put("pinCode", pinCode);
					if(UtilValidate.isNotEmpty(zoneGroupId)){
					orderZone.put("zoneGroupId", zoneGroupId);
					orderZone.put("zoneGroupName", zoneGroupName);
					}else{
						try {
							List<GenericValue>	zoneGroups = delegator.findList("ZoneGroup", null, null, null, null, true);
							GenericValue zoneGroup = EntityUtil.getFirst(zoneGroups);
							if(UtilValidate.isNotEmpty(zoneGroup)){
								orderZone.put("zoneGroupId", zoneGroup.getString("zoneGroupId"));
								orderZone.put("zoneGroupName", zoneGroup.getString("zoneGroupName"));
							}
						} catch (GenericEntityException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
    ...
    public static String checkExternalPayment(HttpServletRequest request, HttpServletResponse response) {
       ...
       /* if(!"icici_full".equals(result))
        	result = "netbanking";*/
        
        //System.out.println("         ICICI          "+result);
        Debug.log("\n\n result ===============aaaaaaa===============  "+result+"\n\n");
        if(!("ICICI_FULL".equalsIgnoreCase(result) || "NETBANKING".equalsIgnoreCase(result)))
    ...
    public static String slotSelection(HttpServletRequest request,
		  	HttpServletResponse response) throws GenericEntityException, IOException, ParseException {
		...
		GenericValue productStore = ProductStoreWorker.getProductStore(request);	
			...
			{
			double cutOffGvtem=productStore.getDouble("cutOffTime");
			cutOffGv=(int)cutOffGvtem;
			}
			List<EntityCondition> condnList=new ArrayList<EntityCondition>();
			
			List<GenericValue> maxDelivery= getAllSlots(delegator);
			...
			if(!UtilValidate.isEmpty(maxDelivery)){
				...
				for(int i=0;i<maxDelivery.size();i++){
					...
					listCount.put(i,0);
					timing=gv.getDouble("maxDelivery").intValue();
				}
			}
			String 	buffer="<td class=label> <span><b>Slot Selection:</b> </span></td><td><select name='slotSelection' id='slotSelection' ><option value='-1'>Select</option>"; 
			
			SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");
			Date d = (Date) sdf.parse(request.getParameter("DeliveryDate"));
			Timestamp SelectedDeliveryDate = new Timestamp(d.getTime());
			Timestamp delivery=UtilDateTime.getDayStart(UtilDateTime.nowTimestamp());
			
			List<EntityCondition> promoConditions = new ArrayList();
			promoConditions.add(EntityCondition.makeCondition("deliveryDate", EntityOperator.EQUALS,SelectedDeliveryDate));
			promoConditions.add(EntityCondition.makeCondition("slotStatus", EntityOperator.NOT_EQUAL,"SLOT_COMPLETED"));
			List ordersList2 = delegator.findList("OrderSlot", EntityCondition.makeCondition(promoConditions, EntityOperator.AND), null, null, null, false);
			List ordersList1=EntityUtil.getFieldListFromEntityList(ordersList2, "partyId", true);
			int r=UtilDateTime.nowTimestamp().getHours();
			
			if(!UtilValidate.isEmpty(ordersList1)){
				for(int i=0;i<ordersList1.size();i++){
					GenericValue gv = (GenericValue) ordersList1.get(i);
						for (int u=0;u<orderType.size();u++)
						{
						if(gv.getString("slotType").equals(orderType.get(u)))
							listCount.put(u,(Integer)listCount.get(u)+1);
						}
					}
					
				}
				if((SelectedDeliveryDate.equals(delivery)) && (r<cutOffGv)) 
				{
					for(int r1=0;r1<orderType.size();r1++)
					{
					 if((Integer)listCount.get(r1)<timing)
						 listOption.add(orderType.get(r1)); 
					}
				}
				
			
				if(!SelectedDeliveryDate.equals(delivery))
				{
				for(int r1=0;r1<orderType.size();r1++)
				{
				...
				if(listOption.size()>=1 ){
					...
					for (int i=0;i<listOption.size();i++) {
						...
						buffer=buffer+"<option value='"+listOption.get(i)+"'>"+listOption.get(i)+"</option>"; 
						}
			   ...
			   out.println(buffer);
				...
				out.println("<b><font size=8 color=red>Nothing is Available</font></b>");
			return "success";
		}
    
    public static String changeSlot(HttpServletRequest request,
		  	HttpServletResponse response) throws GenericEntityException, IOException, ParseException {
    	String slotId=request.getParameter("slotId");
    	String result="error";
    	String deliveryDate=request.getParameter("DeliveryDate");
    	
    	String slotType=request.getParameter("slotSelection");
    	 LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
         Delegator delegator = (Delegator) request.getAttribute("delegator");
         HttpSession session = request.getSession();
         GenericValue userLogin = (GenericValue) session.getAttribute("userLogin");
         GenericValue gv=delegator.findByPrimaryKey("OrderSlot", UtilMisc.toMap("slotId",slotId ));
         GenericValue gv1=delegator.findByPrimaryKey("OrderHeader", UtilMisc.toMap("orderId",gv.getString("orderId") ));
         SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");
			Date d = (Date) sdf.parse(request.getParameter("DeliveryDate"));
			Timestamp deliveryDate1 = new Timestamp(d.getTime());
         if (gv != null)
         {
        	 gv.set("slotType", slotType);
        	 gv.set("deliveryDate", deliveryDate1);
        	 gv.store();
         }
         if (gv1 != null)
         {
        	 gv1.set("slot", slotType);
        	 gv1.set("deliveryDate", deliveryDate1);
        	 gv.store();
        	 result="success";
    ...
    public static String createSlot(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
        ...
        HashMap<String, Object> orderSlotMap = new HashMap();
        orderSlotMap.put("slotTypeId", delegator.getNextSeqId("OrderSlotType"));
        String slotType=request.getParameter("slotType");
        String slotTiming=request.getParameter("slotTiming");
        String maxDelivery=request.getParameter("maxDelivery");
        
        String cutOffTimeInHour = request.getParameter("cutOffTime");
        String cutOffTimeInMinute = request.getParameter("cutOffTimeInMinute");
        
        String timePeriod = request.getParameter("timePeriod");
        String fromDate=request.getParameter("fromDate");
        String thruDate=request.getParameter("thruDate");
        Timestamp fromStamp = null;
        Timestamp thruStamp = null;
        if(UtilValidate.isNotEmpty(slotType)){
        orderSlotMap.put("slotType", slotType);
        }
        if(UtilValidate.isNotEmpty(slotTiming)){
        orderSlotMap.put("slotTiming", slotTiming);
        }
        if(UtilValidate.isNotEmpty(maxDelivery)){
        	orderSlotMap.put("maxDelivery", Double.parseDouble(maxDelivery));
        }
        //Ajaya
        if(UtilValidate.isNotEmpty(cutOffTimeInHour)){
        	if(UtilValidate.isNotEmpty(timePeriod) && "PM".equalsIgnoreCase(timePeriod))
        		orderSlotMap.put("cutOffTimeInHour", Double.parseDouble(cutOffTimeInHour)+12);
   		 	else
   			 	orderSlotMap.put("cutOffTimeInHour", Double.parseDouble(cutOffTimeInHour));
        	
        	orderSlotMap.put("cutOffTime", Double.parseDouble(cutOffTimeInHour));
        }else
        	orderSlotMap.put("cutOffTimeInHour", Double.parseDouble("0"));
        
        if(UtilValidate.isNotEmpty(cutOffTimeInMinute)){
        	orderSlotMap.put("cutOffTimeInMinute", Double.parseDouble(cutOffTimeInMinute));
        }else
        	orderSlotMap.put("cutOffTimeInMinute", Double.parseDouble("0"));
        
        if(UtilValidate.isNotEmpty(timePeriod)){
        	orderSlotMap.put("timePeriod", timePeriod);
        }
        
        //
        if(UtilValidate.isNotEmpty(fromDate)){
        	fromStamp =  new Timestamp(getTimeStamp(fromDate));
        	 orderSlotMap.put("fromDate", fromStamp);
        }
        
        if(UtilValidate.isNotEmpty(thruDate)){
        	thruStamp = new Timestamp(getTimeStamp(thruDate));
        	orderSlotMap.put("thruDate", thruStamp);
          }

       
        
       
        GenericValue orderSlot = delegator.makeValue("OrderSlotType", orderSlotMap);
        try{
        orderSlot.create();
        }
        catch(Exception e){
    ...
    public static long getTimeStamp(String thruDate){
       Date	dateFrom = UtilDateTime.getParsedDate(thruDate, "yyyy-MM-dd HH:mm:ss");
	 
	  return dateFrom.getTime();
    }
    	
    public static String createDeliveryBoy(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
        Delegator delegator = (Delegator) request.getAttribute("delegator");

        HashMap<String, Object> deliveryBoy = new HashMap();
        deliveryBoy.put("deliveryBoyId", delegator.getNextSeqId("DeliveryBoyDetails"));
        deliveryBoy.put("name",request.getParameter("name"));
        deliveryBoy.put("mobileNumber",request.getParameter("mobileNumber"));
        deliveryBoy.put("type",request.getParameter("type"));
        deliveryBoy.put("hiredFrom",request.getParameter("hiredFrom"));
        
        GenericValue orderSlot = delegator.makeValue("DeliveryBoyDetails", deliveryBoy);
        orderSlot.create();
        
        return "success";
    }
    
    public static String updateDeliveryBoy(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
        
        Delegator delegator = (Delegator) request.getAttribute("delegator");
        String deliveryBoyId=request.getParameter("deliveryBoyId");
      
        GenericValue deliveryBoy = delegator.findByPrimaryKey("DeliveryBoyDetails", UtilMisc.toMap("deliveryBoyId",deliveryBoyId ));
       
        if (deliveryBoy != null)
        {
        	deliveryBoy.set("name",request.getParameter("name"));
        	deliveryBoy.set("mobileNumber",request.getParameter("mobileNumber"));
        	deliveryBoy.set("type",request.getParameter("type"));
        	deliveryBoy.set("hiredFrom",request.getParameter("hiredFrom"));
 ...
 public static String updateSlot(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
     ...
     String slotTypeId=request.getParameter("slotTypeId");
     String slotType=request.getParameter("slotType");
     String slotTiming=request.getParameter("slotTiming");
     String maxDelivery=request.getParameter("maxDelivery");
     ...
     if(UtilValidate.isNotEmpty(thruDate)){
      ...
      GenericValue gv=delegator.findByPrimaryKey("OrderSlotType", UtilMisc.toMap("slotTypeId",slotTypeId ));
     ...
     { 
    	 ...
    	 if(UtilValidate.isNotEmpty(slotTiming)){
    		 gv.set("slotTiming", slotTiming); 
    	     
         }
    	 if(UtilValidate.isNotEmpty(maxDelivery)){
    		 gv.set("maxDelivery",Double.parseDouble(maxDelivery)); 
         }
    	 //Ajaya
    	 if(UtilValidate.isNotEmpty(cutOffTimeInHour)){
    		 if(UtilValidate.isNotEmpty(timePeriod) && "PM".equalsIgnoreCase(timePeriod))
    			 gv.put("cutOffTimeInHour", Double.parseDouble(cutOffTimeInHour)+12);
    		 else
    			 gv.put("cutOffTimeInHour", Double.parseDouble(cutOffTimeInHour));
    		 
    		 gv.put("cutOffTime", Double.parseDouble(cutOffTimeInHour));
         }else
        	 gv.put("cutOffTimeInHour", Double.parseDouble("0"));
         
         if(UtilValidate.isNotEmpty(cutOffTimeInMinute)){
        	 gv.put("cutOffTimeInMinute", Double.parseDouble(cutOffTimeInMinute));
         }else
        	 gv.put("cutOffTimeInMinute", Double.parseDouble("0"));
         
    	 if(UtilValidate.isNotEmpty(timePeriod)){
    		 gv.set("timePeriod",timePeriod); 
         }
    	 
    	 //
    	 
    		 gv.set("fromDate", fromStamp); 
    	     
        
    	 
    		 gv.set("thruDate", thruStamp); 
 ...
 public static String checkPostalCode(HttpServletRequest request, HttpServletResponse response) throws GenericEntityException {
        String  flag = "";
	  ...
	  Delegator delegator = (Delegator) request.getAttribute("delegator");
	         ...
	         String location=request.getParameter("locationSearch");
 	         List<GenericValue> locationList=delegator.findList("ZoneType", EntityCondition.makeCondition(EntityOperator.AND,  EntityCondition.makeCondition("pinCode",EntityOperator.EQUALS,location),EntityCondition.makeCondition("thruDate",EntityOperator.EQUALS,null)), null, null, null, false);
     ...
     List<GenericValue> deliveryBoyVehicleDetails = null;
    	 try {
    		 ...
    		 List ConditionList = UtilMisc.toList(
    				 EntityCondition.makeCondition("date", EntityOperator.EQUALS,UtilDateTime.getDayStart(orderHeader.getTimestamp("deliveryDate"))),
    				 EntityCondition.makeCondition("slot", EntityOperator.EQUALS,orderHeader.getString("slot")));
    		 
    		 EntityCondition cond = EntityCondition.makeCondition(ConditionList, EntityOperator.AND);

    		 deliveryBoyVehicleDetails = delegator.findList("DeliveryBoyVehicleDetails", cond,
					 															UtilMisc.toSet("number","slot"), null, null, true);
 ...
 public static List<GenericValue> assignedVehiclesToDeliveryBoy(Delegator delegator){
     List<GenericValue> deliveryBoyVehicleDetails = null;
    	 try {
    		 
    		 EntityCondition cond = EntityCondition.makeCondition("date", EntityOperator.EQUALS,
    				 										UtilDateTime.getDayStart(UtilDateTime.nowTimestamp()));
    		 
    		 EntityFindOptions findOptions = new EntityFindOptions();
             findOptions.setDistinct(true);
    		 deliveryBoyVehicleDetails = delegator.findList("DeliveryBoyVehicleDetails", cond,
					 															UtilMisc.toSet("number"), null, findOptions, true);
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
 ...
 public static List<GenericValue> getAllSlots(Delegator delegator){
     List<GenericValue> slotTypes = null;
    	 try {
		     ...
		     condition.add(EntityCondition.makeCondition("productStoreId",EntityOperator.EQUALS, null));
    		 Debug.log("\n\n EntityCondition.makeCondition(condition,EntityOperator.AND) == "+EntityCondition.makeCondition(condition,EntityOperator.AND)+"\n\n");
    		 slotTypes = delegator.findList("OrderSlotType",EntityCondition.makeCondition(condition,EntityOperator.AND),null, null, null, true);
 ...
 public static List<GenericValue> getAllSlots(Delegator delegator, List slotIds){
     List<GenericValue> slotTypes = null;
    	 try {
    		 List fromDateCond = new ArrayList();
	    	 fromDateCond.add(EntityCondition.makeCondition("fromDate",EntityOperator.EQUALS,null));
		     ...
		     if(UtilValidate.isNotEmpty(slotIds))
		    	 condition.add(EntityCondition.makeCondition("slotType",EntityOperator.IN,slotIds));
