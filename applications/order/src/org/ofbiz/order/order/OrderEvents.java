...
public class OrderEvents {
    ...
    public static String getOrderIdFromHappyCode(HttpServletRequest request, HttpServletResponse response){
    	String happyCode = request.getParameter("happyCode");
    	...
    	if(UtilValidate.isEmpty(happyCode)){
    		if(UtilValidate.isNotEmpty(out))out.print("blankHappyCode");
    		return "success";
    	}
    	...
    	try {
    		List conditionList = UtilMisc.toList(EntityCondition.makeCondition("pinId",EntityOperator.EQUALS,happyCode),
    															EntityCondition.makeCondition("statusId",EntityOperator.EQUALS,"ORDER_DISPATCHED"));
    		orderList = delegator.findList("OrderHeader", 
    							EntityCondition.makeCondition(conditionList,EntityOperator.AND), UtilMisc.toSet("orderId"), null, null, true);
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		if(UtilValidate.isEmpty(orderList)){
			if(UtilValidate.isNotEmpty(out))out.print("orderNotFound");
    		return "success";
    	}
		GenericValue order = EntityUtil.getFirst(orderList);
		if(UtilValidate.isEmpty(order)){
			if(UtilValidate.isNotEmpty(out))out.print("orderNotFound");
    		return "success";
    	}
		out.print(order.getString("orderId"));
    	return "success";
    }
}
