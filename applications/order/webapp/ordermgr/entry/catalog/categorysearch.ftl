...
<h1 style="background:#9aae27; padding:14px 5px 14px 5px; margin:2px">Category Search, <span class="h2">${uiLabelMap.ProductYouSearchedFor}:</span></h1>
...
<#if allCatalogIds?has_content>
    <div class="product-prevnext">
        ...
        <#assign viewIndexMax = Static["java.lang.Math"].ceil((allCatalogIds.size() - 1)?double / viewSize?double)>
        <select name="pageSelect" onchange="window.location=this[this.selectedIndex].value;">
          ...
          <#list 0..viewIndexMax as curViewNum>
            <option value="<@ofbizUrl>categorysearch/~VIEW_SIZE=${viewSize}/~VIEW_INDEX=${curViewNum?int}/~clearSearch=N</@ofbizUrl>">${uiLabelMap.CommonGotoPage} ${curViewNum + 1}</option>
          </#list>
        </select>
        <#-- End Page Select Drop-Down -->
        <b>
        <#if (viewIndex?int > 0)>
          <a href="<@ofbizUrl>categorysearch/~VIEW_SIZE=${viewSize}/~VIEW_INDEX=${viewIndex?int - 1}/~clearSearch=N</@ofbizUrl>" class="buttontext">${uiLabelMap.CommonPrevious}</a> |
        </#if>
        <#if (allCatalogIds.size()?int > 0)>
          <span>${lowIndex+1} - ${highIndex} ${uiLabelMap.CommonOf} ${allCatalogIds.size()}</span>
        </#if>
        <#if highIndex?int < allCatalogIds.size()?int>
          | <a href="<@ofbizUrl>categorysearch/~VIEW_SIZE=${viewSize}/~VIEW_INDEX=${viewIndex+1}/~clearSearch=N</@ofbizUrl>" class="buttontext">${uiLabelMap.CommonNext}</a>
        </#if>
        </b>
    </div>
</#if>
<#if allCatalogIds?has_content>
    ...
    <div class="productsummary-container <#if (numCol?int > 1)> matrix</#if>">
        ...
        <#list allCatalogIds as catalogId>
		<#if catalogId?exists>
     		<#assign CategoryList = Static["org.ofbiz.product.catalog.CatalogWorker"].getProdCatalogCategories(request, catalogId, "PCCT_BROWSE_ROOT")?if_exists>
     		<#if CategoryList?has_content>
     			<#assign browse_root=CategoryList.get(0)>
     			<#if browse_root?has_content>
     			<#assign prodCategory = delegator.findOne("ProductCategory", {"productCategoryId" : browse_root.productCategoryId}, true)?if_exists>
     			<#assign currentCatalogName = Static["org.ofbiz.product.catalog.CatalogWorker"].getCatalogName(request, catalogId)?if_exists>
     			<#if prodCategory?has_content>
				<#if (numCol?int == 1)>
     			<div style="float:left; width:213px; height:200px;">
     			<div style="text-align:center; padding:10px 0 0 0">${currentCatalogName?if_exists}</div>
     			<div style="text-align:center; margin:0 auto; "><a href="<@ofbizUrl>main?CURRENT_CATALOG_ID=${catalogId}</@ofbizUrl>"><img src="${prodCategory.categoryImageUrl?if_exists}" width="200px" height="142px"/></a></div>
     			</div>
				<#else>
