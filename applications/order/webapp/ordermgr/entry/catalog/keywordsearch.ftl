 ...
 document.getElementById("tempSave"+formName).style.display="none";
   ...
   jQuery.ajax({url: "/control/getVariantPrice",
	...
	var value= document.getElementById(add_product_id1).value;
            ...
            if(add_product_id != '' && document.getElementById('productNameDescrip_'+ add_product_id) != '' 
	            && document.getElementById('variant_product_name'+add_product_id) != ''){   
...
<h1 style="background:#CDCEAF; padding:14px 5px 14px 5px; margin:2px">${uiLabelMap.ProductProductSearch}, <span class="h2">${uiLabelMap.ProductYouSearchedFor}: ${searchString?if_exists}</span></h1>
   <#if productIds?has_content && (listSize?int != 1)>
        <form style="float:left;" action="<@ofbizUrl>keywordsearch</@ofbizUrl>" name="sortBy">
    ...
    <div class="product-prevnext" style="margin-top:0px !important;">
        ...
        <select name="pageSelect" onchange="window.location=this[this.selectedIndex].value;">
          ...
          <#list 0..viewIndexMax as curViewNum>
            <option value="<@ofbizUrl>keywordsearch?SEARCH_STRING=${searchString?if_exists}&VIEW_SIZE=${viewSize}&VIEW_INDEX=${curViewNum?int}&clearSearch=N&sortBy=${sortBy?if_exists}&filterBy=${parameters.filterBy?if_exists}&filterByCategory=${parameters.filterByCategory?if_exists}</@ofbizUrl>">${uiLabelMap.CommonGotoPage} ${curViewNum + 1}</option>
          </#list>
        </select>
        <#-- End Page Select Drop-Down -->
        <b>
        <#if (viewIndex?int > 0)>
          <a href="<@ofbizUrl>keywordsearch?SEARCH_STRING=${searchString?if_exists}&VIEW_SIZE=${viewSize}&VIEW_INDEX=${viewIndex?int - 1}&clearSearch=N&sortBy=${sortBy?if_exists}&filterBy=${parameters.filterBy?if_exists}&filterByCategory=${parameters.filterByCategory?if_exists}</@ofbizUrl>" class="buttontext">${uiLabelMap.CommonPrevious}</a> |
        </#if>
        <#if (listSize?int > 0)>
          <span>${lowIndex+1} - ${highIndex} ${uiLabelMap.CommonOf} ${listSize}</span>
        </#if>
        <#if highIndex?int < listSize?int>
          | <a href="<@ofbizUrl>keywordsearch?SEARCH_STRING=${searchString?if_exists}&VIEW_SIZE=${viewSize}&VIEW_INDEX=${viewIndex+1}&clearSearch=N&sortBy=${sortBy?if_exists}&filterBy=${parameters.filterBy?if_exists}&filterByCategory=${parameters.filterByCategory?if_exists}</@ofbizUrl>" class="buttontext">${uiLabelMap.CommonNext}</a>
        </#if>
        </b>
         <#else>
    	<span>${lowIndex+1} - ${highIndex} ${uiLabelMap.CommonOf} ${listSize}</span>
    ...
    <div class="product-prevnext">
        ...
        <#assign viewIndexMax = Static["java.lang.Math"].ceil((listSize - 1)?double / viewSize?double)>
         ...
         <select name="pageSelect" onchange="window.location=this[this.selectedIndex].value;">
          <option value="#">${uiLabelMap.CommonPage} ${viewIndex?int + 1} ${uiLabelMap.CommonOf} ${viewIndexMax + 1}</option>
          <#list 0..viewIndexMax as curViewNum>
            <option value="<@ofbizUrl>keywordsearch?VIEW_SIZE=${viewSize}&VIEW_INDEX=${curViewNum?int}&clearSearch=N&sortBy=${sortBy?if_exists}&filterBy=${parameters.filterBy?if_exists}&filterByCategory=${parameters.filterByCategory?if_exists}</@ofbizUrl>">${uiLabelMap.CommonGotoPage} ${curViewNum + 1}</option>
          </#list>
        </select>
        <#-- End Page Select Drop-Down -->
        <b>
        <#if (viewIndex?int > 0)>
          <a href="<@ofbizUrl>keywordsearch?VIEW_SIZE=${viewSize}&VIEW_INDEX=${viewIndex?int - 1}&clearSearch=N&sortBy=${sortBy?if_exists}&filterBy=${parameters.filterBy?if_exists}&filterByCategory=${parameters.filterByCategory?if_exists}</@ofbizUrl>" class="buttontext">${uiLabelMap.CommonPrevious}</a> |
        </#if>
        <#if (listSize?int > 0)>
          <span>${lowIndex+1} - ${highIndex} ${uiLabelMap.CommonOf} ${listSize}</span>
        </#if>
        <#if highIndex?int < listSize?int>
          | <a href="<@ofbizUrl>keywordsearch?VIEW_SIZE=${viewSize}&VIEW_INDEX=${viewIndex+1}&clearSearch=N&sortBy=${sortBy?if_exists}&filterBy=${parameters.filterBy?if_exists}&filterByCategory=${parameters.filterByCategory?if_exists}</@ofbizUrl>" class="buttontext">${uiLabelMap.CommonNext}</a>
        </#if>
        </b>
         <#else>
    	<span>${lowIndex+1} - ${highIndex} ${uiLabelMap.CommonOf} ${listSize}</span>
