...
if (orderHeader) {
	...
	if(displayParty.partyId)
	{
	personDetails=delegator.findByPrimaryKey("Person", [partyId : displayParty.partyId]);
	context.personDetails= personDetails;
	
	shipping=delegator.findByAnd("OrderContactMech", ['orderId': orderId,'contactMechPurposeTypeId':'SHIPPING_LOCATION']);
	if(shipping != null && shipping.size() > 0)
		shipToName=delegator.findByPrimaryKey("PostalAddress",[contactMechId : shipping.get(0).getString("contactMechId")]);
	}
    ...
    context.orderPaymentPreferencesold = orderPaymentPreferences;
...
/*	getOrderItemsCategoryWise = orderReadHelper.getOrderItemsCategoryWise(delegator,orderItems);
	...
	to remove duplicate key is the list,this code has added
	
	context.keys = getOrderItemsCategoryWise.get("keys");
	keys = getOrderItemsCategoryWise.get("keys");
	...
	if(!keyList.contains(s)){
 	...
 	context.keys = keyList;
	 up to here 

	getOrderItemsCategoryWise = getOrderItemsCategoryWise.remove("keys");
	context.getOrderItemsCategoryWise = getOrderItemsCategoryWise;*/
	
	
	 context.getOrderItemsCategoryWise = orderReadHelper.getOrderItemsCategoryWise(delegator,orderItems);
	Map m1 = new HashMap();
   for(GenericValue gv :orderItems){
	   m1.put(gv.getString("productCategoryId"), orderItems);
		}
   context.sampleMap1=m1;
	
    /*
    orderList = delegator.findList("OrderHeader",null,null,null,null,false);
    
    if(orderList != null)
    for(GenericValue order : orderList)
    	{
    	
    	oredrRole = delegator.makeValue("OrderRole");
    	oredrRole.put("orderId",order.get("orderId"));
    	oredrRole.put("partyId",order.get("partyId"));
    	oredrRole.put("roleTypeId","PLACING_CUSTOMER");
    	print("##############     oredrRole     "+oredrRole);
    	oredrRole.create();
