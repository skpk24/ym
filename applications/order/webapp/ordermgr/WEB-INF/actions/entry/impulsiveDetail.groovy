...
context.productsFromCategoryNextIndex = Integer.parseInt(productsFromCategoryIndex) + 1 ;	
	...
	if(UtilValidate.isNotEmpty(productId) && !productId.equals("GIFTCARD")) {
		...
		{
		/*	conditionList1 = [];
			conditionList1.add(EntityCondition.makeCondition("brandName", EntityOperator.EQUALS, brandName.brandName));
			conditionList1.add(EntityCondition.makeCondition("productId", EntityOperator.NOT_EQUAL, brandName.productId));
			conditionList1.add(EntityCondition.makeCondition("isVariant", EntityOperator.NOT_EQUAL, "Y"));
			if(UtilValidate.isNotEmpty(categoryList))
			conditionList1.add(EntityCondition.makeCondition("productId", EntityOperator.NOT_IN, categoryList.productId));
			if(UtilValidate.isNotEmpty(variant)){
				variant.each { lastTimePeriodHistory ->
				conditionList1.add(EntityCondition.makeCondition("productId", EntityOperator.NOT_EQUAL, lastTimePeriodHistory.productIdTo));
				}
			}
			entityCondition1 = EntityCondition.makeCondition(conditionList1, EntityOperator.AND);
			
			brandList = delegator.findList("Product",entityCondition1 , UtilMisc.toSet("productId"), null, null, false);
		*/
		...
		dontSelectProductids.add(productId)
			if(product.brandName)
			{
				...
				java.util.Set brandSet = new HashSet(brandList);
