...
if (userLogin) {
    exprList = [EntityCondition.makeCondition("partyId", EntityOperator.EQUALS, userLogin.partyId),
			...
			EntityCondition.makeCondition(EntityCondition.makeCondition("shoppingListTypeId", EntityOperator.EQUALS, "SLT_FAV_LIST"), EntityOperator.OR, EntityCondition.makeCondition("shoppingListTypeId", EntityOperator.EQUALS, "SLT_FAV_LIST_AUTO"))];
...
if (product) {
    ...
    //Commented here there is no marketing Package
...
//     context.isMarketingPackage = (isMarketingPackage? "true": "false");
    context.isMarketingPackage = "false";
...
//    }
    ...
    context.productReviews = ratingReviews;
...
//        Number Of Days to ship is option not availbale on youmart 
...
//     There is no DISTINGUISHING_FEAT in youmart
...
//  In youmart we dont have 'STANDARD_FEATURE' and 'SELECTABLE_FEATURE 
...
//    context.sizeProductFeatureAndAppls = sizeProductFeatureAndAppls;
 ...
 //  In youmart we dont have 'Packaging Option' 
    productVariants = [];
...
//adding rows in drop down
                            jsBuf.append("document.forms[\"addform\"].elements[\"FT" + topLevelName + "\"].options[" + (counter) + "] = new Option(\"" + key + "\",\"" + opt + "\");");
   ...
   /* availableInventory = 0.0;
    ...
    // Not required all these assoc for youmart
...
// No Digital Download for youmart 
...
//    context.downloadProductContentAndInfoList = downloadProductContentAndInfoList;
    ...
    //  no get product Iamge for youmart
//    productImageList = [];
