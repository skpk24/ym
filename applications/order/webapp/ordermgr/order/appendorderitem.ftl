  ...
  <div class="screenlet-body">
        <form method="post" action="<@ofbizUrl>appendItemToOrder</@ofbizUrl>" name="appendItemForm">
            ...
            <#if catalogCol?has_content && catalogCol?size == 1>
                <input type="hidden" name="prodCatalogId" value="${catalogCol.get(0)?if_exists}"/>
            </#if>
            <#if shipGroups?size == 1>
                <input type="hidden" name="shipGroupSeqId" value="<#if shipGroups?has_content && shipGroups.get(0)?has_content>${shipGroups.get(0).shipGroupSeqId?if_exists}</#if>"/>
            </#if>
