...
<body>
     ...
     <table>  
      <form id="uploadPartyContent" method="post" enctype="multipart/form-data" action="UpdateDocuments">
...
-->
        ...
        <input type="hidden" name="partyId" value="admin" id="partyId"/>
         <input type="hidden" name="dataResourceId" value="${parameters.dataResourceId?if_exists}" id="dataResourceId"/>
        <input type="hidden" name="partyContentTypeId" value="INTERNAL" id="contentPartyId"/>
        <#assign nowTimestamp = Static["org.ofbiz.base.util.UtilDateTime"].nowTimestamp()>
        <input type="hidden" size='13' name="fromDate" value="${nowTimestamp}"/>
        <tr> <td> 
