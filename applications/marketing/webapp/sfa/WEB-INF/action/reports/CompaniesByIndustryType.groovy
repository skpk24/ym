...
def listist =   delegator.findList("Party",	EntityCondition.makeCondition("partyId", EntityOperator.IN, partyIdList),null, null, null, false)
...
for(j in 0..listist.size()-1){
	...
	opportunityMap = [:]
	opportunityMap.put("createdDate", gv.createdDate.toString())
	
	person = delegator.findByPrimaryKey("PartySummaryCRMView", UtilMisc.toMap("partyId",gv.partyId))	;
	if(!UtilValidate.isEmpty(person)){
		opportunityMap.put("cmpnyName", person.groupName)
	}
    
	status = delegator.findByPrimaryKey("Enumeration", UtilMisc.toMap("enumId",gv.industryType))	;
	if(status)
	opportunityMap.put("industryType", status.description)
	
	
	status = delegator.findByPrimaryKey("Enumeration", UtilMisc.toMap("enumId",gv.industrySubType))	;
	if(status)
	opportunityMap.put("industrySubType", status.description)
...
session.setAttribute("CompaniesByIndustryType_SFA_REPORTS", resultList);