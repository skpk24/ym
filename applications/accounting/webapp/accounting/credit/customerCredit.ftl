         ...
         <#if requestAttributes.customerIdSet?exists && requestAttributes.customerIdSet?has_content>
           ...
           <#assign days=requestAttributes.days?if_exists/>
            ...
            <script>function openCredit(facilityId,frmdate,thrdate,billPartyId){
	            	 window.open("<@ofbizUrl>creditSummary</@ofbizUrl>?facilityId="+facilityId+"&fromDate="+frmdate+"&thruDate="+thrdate+"&billPartyId="+billPartyId, "", "width=800,height=600,status=no,scrollbars=yes");}
	        </script>
 ...
 <#--            <#list trasList as tra>
          ...
          <a href="javascript:openCredit('MyRetailStore','${requestAttributes.fromDateStr?if_exists}','${requestAttributes.thruDateStr?if_exists}','${customerId?if_exists}');" ><u>Total:-${requestAttributes.totalCustomerCredit.get(customerId)?if_exists}
        </u></a>
