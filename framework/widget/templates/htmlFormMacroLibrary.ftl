...
<#macro renderFormatListWrapperOpen formName style columnStyles>
    <table cellspacing="0"   class="<#if style?has_content>${style} dark-grid<#else>basic-table form-widget-table dark-grid</#if>"><#lt/>
</#macro>
...
<#else>
    ...
    <script type="text/javascript">
        jQuery(document).ready(function(){
            new ConstructLookup("${fieldFormName}", "${id}", document.${formName?html}.${name?html}, <#if descriptionFieldName?has_content>document.${formName?html}.${descriptionFieldName}<#else>null</#if>, "${formName?html}", "${width}", "${height}", "center", "${fadeBackground}", <#if ajaxEnabled?has_content && ajaxEnabled>"${ajaxUrl}", "${showDescription}"<#else>"", ""</#if>, "${presentation!}", "${defaultMinLength!2}"<#rt/>
    <#if targetParameterIter?has_content>
      <#assign isFirst = true>
      <#lt/>, [<#rt/>
      <#list targetParameterIter as item>
...
<li class="${paginateFirstStyle}<#if viewIndex gt 0>"><a href="<#if ajaxEnabled>javascript:ajaxUpdateAreas('${ajaxFirstUrl}')<#else>${firstUrl}</#if>"></a><#else>-disabled"><span></span></#if></li>
<li class="${paginatePreviousStyle}<#if viewIndex gt 0>"><a href="<#if ajaxEnabled>javascript:ajaxUpdateAreas('${ajaxPreviousUrl}')<#else>${previousUrl}</#if>"></a><#else>-disabled"><span></span></#if></li>
...
<li class="${paginateNextStyle}<#if highIndex lt listSize>"><a href="<#if ajaxEnabled>javascript:ajaxUpdateAreas('${ajaxNextUrl}')<#else>${nextUrl}</#if>"></a><#else>-disabled"><span></span></#if></li>
<li class="${paginateLastStyle}<#if highIndex lt listSize>"><a href="<#if ajaxEnabled>javascript:ajaxUpdateAreas('${ajaxLastUrl}')<#else>${lastUrl}</#if>"></a><#else>-disabled"><span></span></#if></li>
