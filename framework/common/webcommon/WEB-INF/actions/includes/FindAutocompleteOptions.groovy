...
if (searchFields && fieldValue) {
    ...
    searchFieldsList.each { fieldName ->
        if ("EQUALS".equals(searchType)) {
        	if (searchValue && searchValue instanceof java.util.List) {
            	orExprs.add(EntityCondition.makeCondition(EntityFieldValue.makeFieldValue(searchFieldsList[0]), EntityOperator.IN, searchValue)) 
            	}  
           	else{
