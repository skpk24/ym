...
<#--div class="screenlet">
    ...
    <div class="screenlet-body">
		<form name="salesReportform" method="post" action="<@ofbizUrl>purchaseReoprtCSV</@ofbizUrl>">
			<table class="basic-table eventitle" cellspacing="0">
				...
				<tr>
					<td> Store Id</td>
					<td>
						<select name="storeId" onchange="changeRequest('salesReportform', this.value)">
...
<script>
	function changeRequest(form, storeId){
		var form=document[form];
		if(storeId == '9000'){
			form.action = '<@ofbizUrl>purchaseReoprtCSV</@ofbizUrl>';
		}else{
			form.action = '<@ofbizUrl>reportToSupplier</@ofbizUrl>';
		}
	}
</script>
