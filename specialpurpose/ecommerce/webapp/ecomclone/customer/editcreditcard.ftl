...
<#else>
  <#if !creditCard?exists>
      ...
      <form method="post" class="creditcard-info" action="<@ofbizUrl>createCreditCard?DONE_PAGE=${donePage}</@ofbizUrl>" name="editcreditcardform">
      <div>
    <#else>
      <h2>${uiLabelMap.AccountingEditCreditCard}</h2>
      <form method="post" class="creditcard-info" action="<@ofbizUrl>updateCreditCard?DONE_PAGE=${donePage}</@ofbizUrl>" name="editcreditcardform">
        <div>
        <input type="hidden" name="paymentMethodId" value="${paymentMethodId}" />
    </#if>
      <a href="<@ofbizUrl>${donePage}</@ofbizUrl>" class="buttontext">${uiLabelMap.CommonCancelDone}</a>
      <a href="javascript:document.editcreditcardform.submit()" class="buttontext">${uiLabelMap.CommonSave}</a><br/><br/>
      <table width="90%" border="0" cellpadding="2" cellspacing="0" style="font-size:12px;">
         ...
         <tr>
           <td align="right" valign="top" style="font-weight:bold;">${uiLabelMap.PartyBillingAddress}</td>
           <td>
        <#-- Removed because is confusing, can add but would have to come back here with all data populated as before...
        <a href="<@ofbizUrl>editcontactmech</@ofbizUrl>" class="buttontext">
          [Create New Address]</a>&nbsp;&nbsp;
        -->
             <table width="100%" border="0" style="font-size:12px;" cellpadding="1">
