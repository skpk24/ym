...
<div id="microcart">
        <#if (shoppingCartSize > 0)>
              <a href="<@ofbizUrl>showcart</@ofbizUrl>"> <#--${uiLabelMap.EcommerceCartHas}--> <strong id="microCartQuantity">${shoppingCartSize}</strong>
                <#if shoppingCart.getTotalQuantity() == 1>${uiLabelMap.OrderItem}<#else/>${uiLabelMap.OrderItems}</#if>,
                <#--<@ofbizCurrency amount=shoppingCart.getDisplayGrandTotal() isoCode=shoppingCart.getCurrency()/>--><span class="WebRupee">&#8377;</span>&nbsp;<span id="microCartTotal">${shoppingCart.getDisplayGrandTotal()?if_exists}</span></a>
            <span id="microCartEmpty" style="display:none">${uiLabelMap.OrderShoppingCartEmpty}</span>
        <#else>
       <a href="/control/showcart"  id="abcxyzhref"><span id="microCartQuantity">0</span> Items,</a><span class="WebRupee">&#8377;</span>&nbsp;<span id="microCartTotal">0.00</span>
        </#if>
    <#-->ul>
