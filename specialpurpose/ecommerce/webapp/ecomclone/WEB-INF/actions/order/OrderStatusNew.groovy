...
condnList.add(EntityCondition.makeCondition("orderId",EntityOperator.EQUALS,orderId));
condnList.add(EntityCondition.makeCondition("paymentMethodTypeId",EntityOperator.EQUALS,"EXT_BILLACT"));
//Billing Amount
List<GenericValue> billList=delegator.findList("OrderPaymentPreference",EntityCondition.makeCondition(condnList,EntityOperator.AND),null,null,null,false);

GenericValue BillAmt=EntityUtil.getFirst(billList);
List condnList1=new ArrayList();
condnList1.add(EntityCondition.makeCondition("orderId",EntityOperator.EQUALS,orderId));
condnList1.add(EntityCondition.makeCondition("paymentMethodTypeId",EntityOperator.EQUALS,"EXT_INVITE"));
//invite Amount
List<GenericValue> inviteList=delegator.findList("OrderPaymentPreference",EntityCondition.makeCondition(condnList1,EntityOperator.AND),null,null,null,false);

GenericValue inviteAmt=EntityUtil.getFirst(inviteList);
...
if (orderHeader) {
    ...
    orderSubTotal = orderReadHelper.getOrderItemsSubTotalNew();
    
    isOrderContainsGiftCard = orderReadHelper.isOrderContainsGiftCard();
	...
	List<String> paymentMth = new ArrayList<String>();
	for(GenericValue abc : orderPaymentPreferences)
	{
		String PaymentId=(String)abc.get("paymentMethodTypeId");
		paymentMth.add(PaymentId);
	}
	context.paymentMth = paymentMth;
	 paymentMethods = [];
	...
	// Start Code for find No Of Items for particular order
	List<GenericValue> prodIds=new ArrayList();
	List prodCatIds=new ArrayList();
	Double  totalOrderedItems=0;
	paymentAppls1 = delegator.findByAnd("OrderItem", [orderId : orderId]);
	
	for (GenericValue returnItem : paymentAppls1) {
		 totalOrderedItems = returnItem.getDouble("quantity");
		//Double returnQuantityDbl = returnItem.getDouble("returnQuantity");
		if (UtilValidate.isNotEmpty(totalOrderedItems)) {
			
			prodCatIds.add(totalOrderedItems);
		}
	}
	
	Double itemSum = 0.0;
	for(int i=0;i<prodCatIds.size();i++)
	{
		itemSum=itemSum+(Double)prodCatIds.get(i);
	}
	
	context.noOfItems = itemSum;
	// End Code for find No Of Items for particular order
	
	
	// Start Code for find Contact No for particular customer
	showOld = "true".equals(parameters.SHOW_OLD);

	createdBy = orderHeader.getString("createdBy");


	userLogin = delegator.findByPrimaryKeyCache("UserLogin", UtilMisc.toMap("userLoginId", createdBy ));
	
	partyContactMechValueMaps = ContactMechWorker.getPartyContactMechValueMaps(delegator, userLogin.partyId, showOld);
	context.partyDetailsMap = org.ofbiz.party.party.PartyHelper.getShippingContactdeatils(delegator, userLogin.partyId);
	context.partyContactMechValueMaps =partyContactMechValueMaps;
	
	// End Code for find Contact No for particular customer
    ...
    context.isOrderContainsGiftCard = isOrderContainsGiftCard;
	...
	if(!UtilValidate.isEmpty(BillAmt))
	{
    context.billingAmt=BillAmt.getDouble("maxAmount");
    context.total=(OrderReadHelper.getOrderGrandTotal(orderItems, orderAdjustments))-BillAmt.getDouble("maxAmount");
	}
	
	if(!UtilValidate.isEmpty(inviteAmt))
	{
	context.inviteRef=inviteAmt.getDouble("maxAmount");
	context.totalAmount=(OrderReadHelper.getOrderGrandTotal(orderItems, orderAdjustments))-inviteAmt.getDouble("maxAmount");
	}
