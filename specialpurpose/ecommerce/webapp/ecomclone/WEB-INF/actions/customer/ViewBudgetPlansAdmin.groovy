...
if(partyId)
{
	...
	ecl1 = EntityCondition.makeCondition([
		...
		EntityCondition.makeCondition("changeByUserLoginId", EntityOperator.EQUALS, userData.get(0).get("userLoginId"))],
	EntityOperator.AND);
	...
	while (budPlanLst.hasNext())
	{
		...
		if(budgetItm.get("productCategoryId") !=  null)
		{
			amt = java.math.BigDecimal.ZERO;
			if(pIds.containsKey(budgetItm.get("productCategoryId")))
			{
				budAmt = pIds.get(budgetItm.get("productCategoryId"));
			...
			pIds.put(budgetItm.get("productCategoryId"), amt);
		}
	}
