...
//Get Cart Items
// Already in the ShowCar page ew are querying
...
qtyMap = ShoppingCartEvents.totalQtyAddedInCartList(request);
msg = "";
indexes_qty = "";
if(qtyMap != null)
{
	keys = qtyMap.keySet();
	keys.each { key ->
		msg = msg + qtyMap.get(key) + "\n";
		indexes_qty = indexes_qty+key+",";
	}
	context.msg = msg;
	context.indexes_qty = indexes_qty;
}
