...
citrusBankId = request.getParameter("citrusBankId");
errorMessages = [];
errorMaps = [:];
attributeMap = UtilHttp.getAttributeMap(request);
println("  paymentMethodTypeId          "+paymentMethodTypeId);
if (paymentMethodTypeId) {
    
	if(paymentMethodTypeId.equalsIgnoreCase("CREDIT_CARD")){
		if(attributeMap.paymentMethodId)
		paymentMethodId = attributeMap.paymentMethodId;
		else
    ...
    if (paymentMethodTypeId.equals("EXT_NETBANKING")) {
    	cpi = cart.getPaymentInfo(paymentMethodId, citrusBankId, null, null, null, true);
    }else{
